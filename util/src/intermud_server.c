...
void net_send_mudlist(struct dns_record *remote_mud)
{
    char mudlist_buffer[2000];
    char buf[500];
    int  num_muds = 0;
    struct in_addr temp;
    struct dns_record *dummy;
    struct sockaddr_in remote_addr;
    char *mud_services;
   
    strcpy(mudlist_buffer, "1050|");
   
    for (dummy = dns_table; dummy; dummy = dummy->next)
	if (str_cmp(dummy->mud_name, remote_mud->mud_name) != 0) {
	    temp.s_addr = dummy->ip_address;
      
	    mud_services = encode_services(dummy);
      
	    sprintf(buf, "%s|%s|%s|%d|%d|%s|%s|%s|", dummy->mud_name, dummy->mud_port, 
		    inet_ntoa(temp), ntohs(dummy->port),
		    dummy->mud_type, dummy->mud_version,
		    dummy->intermud_version, mud_services);
	    strcat(mudlist_buffer, buf);
	    num_muds++;
	}
  
    if (num_muds == 0)
	strcat(mudlist_buffer, "FIRST|");
  
    DEBUG(("INFO: MUDLIST = %s\r\n", mudlist_buffer));
   
    bzero((char *) &remote_addr, sizeof(remote_addr));
    remote_addr.sin_family = AF_INET;
    remote_addr.sin_addr.s_addr = remote_mud->ip_address;
    remote_addr.sin_port = remote_mud->port;

    DEBUG(("SNET: [MUDLIST  ] %s\n", mudlist_buffer));
  
    sendto(net_sockfd, mudlist_buffer, strlen(mudlist_buffer), 0, (struct sockaddr *) &remote_addr, 
	   sizeof(remote_addr));
}
...
void net_keep_alive(void)  
{
    struct dns_record *dummy, *temp;
    struct sockaddr_in remote_address;
    char buffer[160];
    char *Services_Buffer;
...
void recv_net_message(void)
{
	...
	switch(msgnum) {
	case 1000: net_recv_startup(message, remote_address); break;
	case 1020: net_recv_pingreq(message, remote_address); break;
	case 1030: net_recv_pingrpy(message, remote_address); break;
	case 1045: net_recv_mudlist_req(message, remote_address); break;
...
void net_recv_startup(char *net_message, struct sockaddr_in remote_address)
{
    struct dns_record *dummy, *dns, dummy1;
    struct in_addr temp;
    int   found = 0;
  
    DEBUG(("RNET: [STARTUP  ] %s\n", net_message));
  
    Message_Type = strtok(net_message, "|");
    Intermud_Port = strtok(NULL, "|");
    Mud_Name = strtok(NULL, "|");
    ...
    if (Message_Type == NULL || Mud_Name == NULL || Mud_Type == NULL ||
	Mud_Version == NULL || Intermud_Version == NULL || 
	Mud_Port == NULL || Services == NULL)
	interlog("ERRO: Startup message from %s failed sanity check\n", 
		 inet_ntoa(remote_address.sin_addr));
    else {
	for (dns = dns_table; dns; dns = dns->next)
	    if (dns->ip_address == remote_address.sin_addr.s_addr && 
		dns->port == htons(atoi(Intermud_Port)))  {
		found = 1;
		break;
	    }
	if (found == 1)  {
	    interlog("INFO: Received startup message for mud already in dns_table\n");
       
	    dummy1.ip_address = remote_address.sin_addr.s_addr;
	    dummy1.port = htons(atoi(Intermud_Port));
	    strcpy(dummy1.mud_name, Mud_Name);
	    strcpy(dummy1.mud_port, Mud_Port);
	    strcpy(dummy1.mud_version, Mud_Version);
	    strcpy(dummy1.intermud_version, Intermud_Version);
	    dummy1.mud_type = atoi(Mud_Type);
	    dummy1.services = 0;
      
	    decode_services(Services, &dummy1);
      
	    temp.s_addr = dummy1.ip_address;
     
	    DEBUG(("INFO: Received startup message from %s, IP = %s\n", 
		   Mud_Name,
		   inet_ntoa(temp)));
	    net_send_mudlist(&dummy1);
	}
	else  {
	    ...
	    strcpy(dummy->mud_name, Mud_Name);
...
void net_recv_mudlist_req(char *net_message, struct sockaddr_in remote_address)
{
    ...
    DEBUG(("RNET: [MUDLISTRQ] %s\n", net_message));
  
    Service = strtok(net_message, "|");
    Remote_Mud = strtok(NULL, "|");
  
    if (Service != NULL && Remote_Mud != NULL)  {
	for (dns = dns_table; dns; dns = dns->next)
	    if (dns->ip_address == remote_address.sin_addr.s_addr)  {
		dummy = dns;
		found = 1;
	...
	if (found == 1)  {
	    ...
	    interlog("INFO: Received MUDLISTREQ from unknown MUD, IP = %s\r\n", 
		     inet_ntoa(remote_address.sin_addr));
    }
    else
	interlog("ERRO: Received ambiguous MUDLISTREQ from %s\r\n", 
		 inet_ntoa(remote_address.sin_addr));
}
...
struct dns_record *check_dns_record(struct sockaddr_in remote_address)
{
    int                found = 0;
    char               buffer[50];
    struct dns_record *dns_temp, *dummy;
    char *Services_Buffer;
