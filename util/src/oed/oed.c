...
void 
convert (struct obj_file_elem *oldob, struct NEWobj_file_elem *newob)
{
  int i;
  ...
  newob->bitvector[0]     = oldob->bitvector[0];
  newob->bitvector[1]     = oldob->bitvector[1];
  newob->bitvector[2]     = oldob->bitvector[2];
...
main (int argc, char **argv)
{
	...
	break;
      ...
      fread(&rent_header, sizeof(struct rent_info), 1, infile);
      fwrite(&rent_header, sizeof(struct rent_info), 1, outfile);      
