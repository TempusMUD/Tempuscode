...
Crash_delete_file( char *name, int mode )
{
	...
	if ( errno != ENOENT ) {	/* if it fails but NOT because of no file */
	    sprintf( buf1, "SYSERR: deleting %s file %s ( 1 )", mode ? "implant" :
		     "crash", filename );
	    perror( buf1 );
	}
	return 0;
	...
	if ( errno != ENOENT ) {	/* if it fails, NOT because of no file */
	    sprintf( buf1, "SYSERR: deleting %s file %s ( 2 )", mode ? "implant" :
		     "crash", filename );
	    perror( buf1 );
	}
    }
...
Crash_clean_file( char *name )
{
	...
	if ( errno != ENOENT ) {	/* if it fails, NOT because of no file */
	    sprintf( buf1, "SYSERR: OPENING OBJECT FILE %s ( 4 ) [%d: %s]", fname, errno, strerror( errno ) );
	    perror( buf1 );
	}
	return 0;
    }
...
Crash_listrent( struct char_data * ch, char *name )
{
	...
	sprintf( buf, 
		 "%s %2d items :: %s%7d%s/day  %s%8d%s gold.  %s%8d%s bank\r\n", 
		 buf, 0, //rent.nitems,
	...
	if ( ( obj = real_object_proto( object.item_number ) ) ) {
	    sprintf( buf, 
		     "%s %s[%s%5d%s]%s %s( %s%5dau%s )%s %s%-30s%s%s%s%s%s%s%s\r\n", 
		     ...
		     CCYEL( ch, C_NRM ), CCNRM( ch, C_NRM ),
		     CCGRN( ch, C_NRM ), 
		     strlen( object.short_desc ) ? object.short_desc : 
		     obj->short_description,
		     CCNRM( ch, C_NRM ), IS_SET( object.extra2_flags, ITEM2_BROKEN ) ?
		     "<brk>" : "",
		     ...
		     long cost )
{
    ...
    sprintf( buf2, "You can rent for %ld day%s with the money you have on hand and in the bank.",
		  ...
		  int display, int factor )
{
    ...
    if ( !numitems ) {
	perform_tell( receptionist, ch, "But you are not carrying anything!  Just quit!" );
	return ( 0 );
    }
		  ...
		  int cmd, char *arg, int mode )
{
    ...
    if ( free_rent ) {
	perform_tell( recep, ch, "Rent is free here.  Just quit, and your objects will be saved!" );
	return 1;
    }
