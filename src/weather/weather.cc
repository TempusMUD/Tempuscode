...
another_hour(int mode)
{
	...
	lunar_phase = (((lunar_stage == 0) || (lunar_stage == 23)) ? MOON_NEW :
		       ...
		       (lunar_stage < 19) ? MOON_LAST_QUARTER :
		       MOON_WANE_CRESCENT);
	...
	if (local_time.hours == (4 - daylight_mod[(int)local_time.month])) {
		...
		send_to_zone("The blue sun rises in the cloudless skies of the east.\r\n", zone, 1);
	    else if (zone->weather->sky == SKY_CLOUDY)
		send_to_zone("Through the veil of clouds, you see a glimmer of blue to the east.\r\n", zone, 1);
	    else if (zone->weather->sky == SKY_RAINING)
		send_to_zone("The faint light of the blue sun appears through the rain to the east.\r\n", zone, 1);
	    else
		send_to_zone("A small blue sun rises in the east.\r\n", zone, 1);

	    /* SUN_RISE */
	} else if (local_time.hours == (5 - daylight_mod[(int)local_time.month])) {
	    zone->weather->sunlight = SUN_RISE;
	    if (!number(0, 2))
		send_to_zone("The huge red sun rises over the eastern horizon.\r\n", 
			     zone, 1);
	    else if (zone->weather->sky == SKY_CLOUDLESS)
		send_to_zone("The blazing red giant rises into the clear sky.\r\n", 
			     zone, 1);
	    else if (zone->weather->sky == SKY_CLOUDY)
		send_to_zone("The red light of the sun appears in the cloudy sky.\r\n",
			     zone, 1);
	    else
		send_to_zone("The huge red sun rises over the eastern horizon.\r\n", 
			     zone, 1);

	} else if (local_time.hours == (6 - daylight_mod[(int)local_time.month])) {
	    zone->weather->sunlight = SUN_LIGHT;
	    if (!number(0, 2))
		send_to_zone("The day has begun, both suns above the horizon.\r\n", zone, 1);
	    else if (zone->weather->sky == SKY_CLOUDLESS)
	...
	} else if (local_time.hours == (20 + daylight_mod[(int)local_time.month])) {
	    zone->weather->sunlight = SUN_SET;
	    send_to_zone("The red giant sun slowly sets in the west.\r\n", zone, 1);
      
	} else if (local_time.hours== (21 + daylight_mod[(int)local_time.month])) {
	    zone->weather->sunlight = SUN_SET;
	    send_to_zone("The red giant sun slowly sets in the west.\r\n", zone, 1);
      
	} else if (local_time.hours== (22 + daylight_mod[(int)local_time.month])) {
...
jet_stream()
{
	...
	for (zone = zone_table; zone; zone = zone->next) {
		...
		count++;
		change += 
		    ((fr_zone->weather->pressure + zone->weather->pressure) / 2) -
...
zone_weather_change(struct zone_data *zone)
{
	...
	if (!number(0, 2))
	    ...
	    send_to_zone("As the suns dip below the horizon, clouds cover the sky.\r\n", zone, 1);
	...
	if (!number(0, 2))
	    ...
	    send_to_zone("Rain begins to fall across the land, as the double suns rise.\r\n", zone, 1);
	else if (zone->weather->sunlight == SUN_SET)
	    send_to_zone("As the suns sink below the horizon, rain begins to fall.\r\n", zone, 1);
	else if (zone->weather->sunlight == SUN_DARK)
	...
	if (!number(0, 2))
	    ...
	    send_to_zone("The last of the clouds in the sky vanish with the sunset.\r\n", zone, 1);
	else if (zone->weather->sunlight == SUN_DARK)
	    send_to_zone("The clouds part, leaving a clear view of the stars.\r\n", zone, 1);
	else
	...
	if (!number(0, 2))
	    ...
	    send_to_zone("Lightning and thunder begin to accompany the sunrise.\r\n", zone, 1);
	else if (zone->weather->sunlight == SUN_SET)
	    send_to_zone("As the suns set, lightning begins to flicker across the sky.\r\n", zone, 1);
	else if (zone->weather->sunlight == SUN_DARK)
	    send_to_zone("The starry sky is lit by flashes of lightning.\r\n", zone, 1);
	else
	    send_to_zone("You can see lightning flickering across the sky.\r\n", zone, 1);
	...
	if (!number(0, 2))
	    ...
	    send_to_zone("The rain ceases as the twin suns rise above the horizon.\r\n", zone, 1);
	else if (zone->weather->sunlight == SUN_SET)
	    send_to_zone("The rain ceases as the twin suns sink below the horizon.\r\n", zone, 1);
	else if (zone->weather->sunlight == SUN_DARK)
	...
	if (!number(0, 2))
	    ...
	    send_to_zone("The lightning ceases as the suns rise on a rainy day.\r\n", zone, 1);
	else if (zone->weather->sunlight == SUN_SET)
