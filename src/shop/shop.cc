...
same_obj(struct obj_data * obj1, struct obj_data * obj2)
{
    ...
    if ((obj1->shared->proto && 
        ...
        (obj2->shared->proto &&
         (obj2->short_description != obj2->shared->proto->short_description ||
...
struct obj_data *
get_slide_obj_vis(struct char_data * ch, char *name,
...
struct obj_data *
get_hash_obj_vis(struct char_data * ch, char *name,
		 struct obj_data * list)
{
	      ...
	      struct char_data * keeper, struct shop_data *shop)
{
    ...
    while ((obj) && (GET_MONEY(keeper, shop) + SHOP_BANK(shop) >=
                     ( obj_selling_price = sell_price(ch, obj, shop) ) ) && (sold < sellnum)) {
        sold++;
	       ...
	       struct char_data * keeper, struct shop_data *shop)
{
    ...
    sprintf(buf, "I'll give you %d %s for that!", sell_price(ch, obj, shop), 
            (SHOP_CURRENCY(shop) == CURRENCY_CREDITS) ? "credits" : 
            "gold coins");
    perform_tell(keeper, ch,  buf);
	    ...
	    int index, struct shop_data *shop)
{
    ...
    if (shop_producing(obj, shop))
        ...
        sprintf(buf2, "%s%5d%s       ", CCYEL(ch, C_NRM), cnt, CCNRM(ch, C_NRM));
    sprintf(buf, " %2d%s)%s  %s", index, CCRED(ch, C_NRM),CCNRM(ch,C_NRM),buf2);
	      ...
	      struct char_data * keeper, struct shop_data *shop)
{
    ...
    strcat(buf, " ##   Available   Item                                               Cost\n\r");
    strcat(buf, "-------------------------------------------------------------------------\n\r");
...
SPECIAL(shop_keeper)
{
    ...
    if (!CMD_IS("moan") && !CMD_IS("gossip") && !CMD_IS("holler") && 
        ...
        if (keeper->followers) {
            ...
            if (IS_AFFECTED(sucker, AFF_CHARM) && sucker->master && 
                sucker->master == keeper && sucker->in_room == keeper->in_room) {
                switch(number(0, 60)) {
                case 0:
                ...
                case 2:
                    if (GET_GOLD(sucker)) {
                        ...
                        act("$n convinces $N to give $m a loan.", 
                            FALSE, keeper, 0, sucker, TO_NOTVICT);
                        GET_GOLD(sucker) = MAX(0, GET_GOLD(sucker) - GET_LEVEL(keeper));
                    }
                    break;
                case 3:
                    sprintf(buf2, "%s gossip I am a fool.  I tried to charm %s.", 
                            GET_NAME(sucker), GET_NAME(keeper));
                    do_order(keeper, buf2, 0, 0);
                    break;
                case 4:
                    sprintf(buf2, "%s gossip Anybody want anything from the store?", 
                            GET_NAME(sucker));
                    do_order(keeper, buf2, 0, 0);
                    break;
                case 5:
                    sprintf(buf, CAP(GET_NAME(keeper)));
                    sprintf(buf2,"%s holler WOW!! %s is so great!",GET_NAME(sucker), buf);
                    do_order(keeper, buf2, 0, 0);
                    break;
                case 6:
    ...
    if (CMD_IS("buy")) {
        ...
        if (!isname(buf, keeper->player.name))
            ...
            perform_tell(keeper, ch, "Just type LIST. Then you buy them.. NO?");
...
boot_the_shops(FILE * shop_f, char *filename, int rec_count)
{
    ...
    while (!done) {
        ...
        if (*buf == '#') {		/* New shop */
            ...
            temp = read_list(shop_f, list, new_format, MAX_PROD, LIST_PRODUCE, new_shop);
...
list_detailed_shop(struct char_data * ch, struct shop_data *shop)
{
    ...
    sprintf(buf, "Buy at:     [%4.2f], Sell at: [%4.2f], Open: [%d-%d, %d-%d], Currency: %s%s%s\r\n",
            SHOP_BUYPROFIT(shop), SHOP_SELLPROFIT(shop), SHOP_OPEN1(shop),
            SHOP_CLOSE1(shop), SHOP_OPEN2(shop), SHOP_CLOSE2(shop),
            CCCYN(ch, C_NRM), 
            SHOP_CURRENCY(shop) == CURRENCY_CREDITS ? "CREDITS" : "GOLD",
            CCNRM(ch, C_NRM));
    ...
    sprintf(buf, 
            ...
            QRED, QNRM, shop->message_buy,
            QRED, QNRM, shop->message_sell);
    send_to_char(buf, ch);
}
