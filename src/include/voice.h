...
public:
    ...
    void perform(Creature *ch, thing *vict, voice_situation situation);

    int _idnum;
    std::string _name;
    std::map<int, std::vector< std::string > > _emits;
};

extern std::map<int,Voice> voices;
...
void emit_voice(Creature *ch, thing *vict, voice_situation situation);
