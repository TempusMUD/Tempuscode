...
class Question {
    public:
        ...
        const char* getQuestion() { return question.c_str(); }
            // Returns gen
        int getGen()  { return gen; }
            // Returns point value
        int getValue(){ return value; }
            // Returns the requested choice
        const char* getChoice(int index) { return choices[index].c_str(); }
            // Returns the # of choices available
        int getChoiceCount() { return choices.size(); }
...
Question::Question(xmlNodePtr n,xmlDocPtr doc) {
  ...
  // Returns true if guess is a correct answer
bool Question::isAnswer(const char *guess) {
...
**/
static inline bool load_remort_questions() {
...
class Quiz : private vector<Question*> {
    public:
        ...
        void reset();
        bool inProgress() { return studentID != 0; }
        ...
        bool isReady() { return (size() == 0 && studentID == 0); }
            // Send test statistics to ch
        void sendStatus(char_data *ch);
        void sendQuestion(char_data *ch);
        bool isPassing() { return earnedPoints >= neededPoints; }
        int getScore() { 
            if( earnedPoints == 0 ) 
                return 0;
