...
load_elevators(void)
{
	...
	if (!get_line(file, line)) {
	    sprintf(buf, "SYSERR: Format error in elevator file, after %d", last);
	    slog(buf);
	    fclose(file);
	    return 0;
	}
	...
	if (nr > last) {
	    sprintf(buf, "SYSERR: elevator %d is after %d.. must decrease.",nr,last);
	    slog(buf);
	    fclose(file);
	    return 0;
	}
	...
	while (1) {
	    if (!get_line(file, line)) {
		sprintf(buf, "SYSERR: Format error in elevator file, in %d", nr);
		slog(buf);
		while ((elem = head->list)) {
		    head->list = elem->next;
		    if (elem->name)
...
target_room_on_queue(int vnum, struct elevator_data *elev)
{
    ...
    for (lnk = elev->pQ; lnk; lnk = lnk->next)
	if (lnk->phead && lnk->phead->path &&
	    lnk->phead->path[0].data == vnum)
	    return 1;
    return 0;
}
int
