...
ACMD(do_steal)
{
	...
	send_to_char("The universal forces of order prevent those acts here.\r\n",
		     ch);
	...
	if (!(obj = get_obj_in_list_vis(ch, obj_name, vict->carrying))) {
		...
		if (vict->getPosition() > POS_SLEEPING) {
			...
			if (vict->getPosition() == POS_SLEEPING) {
				...
				FALSE, ch, 0, vict, TO_CHAR);
			    act("You are disturbed as $n attempts to pilfer your inventory.",
	...
	if (AWAKE(vict) && (percent > CHECK_SKILL(ch, SKILL_STEAL)) && 
		...
		send_to_char(buf, ch);
		if (GET_LEVEL(ch) >= LVL_AMBASSADOR) {
		    sprintf(buf, "%s stole %d coins from %s.", GET_NAME(ch), gold, GET_NAME(vict));
		    slog(buf);
		}
	    } else {
...
ACMD(do_circle)
{
	...
	send_to_char("You can't circle someone who is actively fighting you!\r\n", ch);
	return;
    }
...
ACMD(do_hide)
{
	...
	send_to_char("...but it sure will be hard with all that heavy equipment.\r\n", ch);
	percent += 30;
    }
    ...
    if (percent > CHECK_SKILL(ch, SKILL_HIDE) + dex_app_skill[GET_DEX(ch)].hide)
	return;
