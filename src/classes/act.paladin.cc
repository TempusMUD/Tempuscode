//
// File: act.paladin.c                     -- Part of TempusMUD
//
...
ACMD(do_holytouch)
{
	...
	if (ch->checkSkill(SKILL_HOLY_TOUCH) < 40 || IS_NEUTRAL(ch)) {
...
/*	int chance = 0;
	...
	if (vict->affectedBy(SKILL_HOLY_TOUCH)
		|| vict->affectedBy(TYPE_MALOVENT_HOLYTOUCH)) {
	...
	if (AFF_FLAGGED(vict, AFF_PROTECT_EVIL))
		chance -= 20;
	if (vict->affectedBy(SPELL_BLESS))
		chance -= 10;
	if (vict->affectedBy(SPELL_PRAY))
		chance -= 10;
...
/*	int mod, gen;
	...
	mod = (GET_LEVEL(ch) + gen + ch->checkSkill(SKILL_HOLY_TOUCH) / 16);
	...
	if (GET_MOVE(ch) > mod) {
		...
		if (IS_GOOD(ch)) {
			if (gen > 0 && vict->affectedBy(TYPE_MALOVENT_HOLYTOUCH))
				affect_from_char(vict, TYPE_MALOVENT_HOLYTOUCH);
			if (gen > 1) {
				if (vict->affectedBy(SPELL_SICKNESS))
					affect_from_char(vict, SPELL_SICKNESS);
				REMOVE_BIT(AFF2_FLAGS(vict), AFF3_SICKNESS);
			}
			if (gen > 2 && vict->affectedBy(SPELL_POISON))
				affect_from_char(vict, SPELL_POISON);
			if (gen > 3 && vict->affectedBy(TYPE_RAD_SICKNESS))
				affect_from_char(vict, TYPE_RAD_SICKNESS);
			if (gen > 4 && vict->affectedBy(SPELL_BLINDNESS))
				affect_from_char(vict, SPELL_BLINDNESS);
			if (gen > 5 && vict->affectedBy(SKILL_GOUGE))
				affect_from_char(vict, SKILL_GOUGE);
			if (gen > 6 && vict->affectedBy(SKILL_HAMSTRING))
				affect_from_char(vict, SKILL_HAMSTRING);
			if (gen > 7 && vict->affectedBy(SPELL_CURSE))
				affect_from_char(vict, SPELL_CURSE);
			if (gen > 8 && vict->affectedBy(SPELL_MOTOR_SPASM))
				affect_from_char(vict, SPELL_MOTOR_SPASM);
			if (gen > 9) {
				if (vict->affectedBy(SPELL_PETRIFY))
