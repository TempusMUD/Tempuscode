...
do_simple_move(struct char_data * ch, int dir, int mode, int need_specials_check)
{
    ...
    if (GET_POS(ch) == POS_FLYING && SECT_TYPE(ch->in_room) != SECT_INSIDE &&
	...
	for (tch = ch->in_room->people; tch; tch = tch->next_in_room) {
	    ...
	    else if (GET_POS(ch) < POS_FLYING && GET_LEVEL(ch) < LVL_AMBASSADOR &&
	...
	if (GET_POS(ch) == POS_STANDING && IS_AFFECTED_3(ch, AFF3_HAMSTRUNG)) {
		send_to_char("The gash in your leg reopens!\r\n",ch);
