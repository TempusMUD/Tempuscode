...
void reply_respond(struct char_data *ch, struct char_data *vict, char * desc)
{
    ...
    {
	desc2++;
	if (desc2[0] == 'G') rpl_give_gold(ch, vict, desc2);
	if (desc2[0] == 'O') rpl_give_obj (ch, vict, desc2);
	if (desc2[0] == 'U') rpl_user_spec(ch, vict, desc2);

    } else
    {
	sprintf(buf_temp, "%s%s tells you:%s\r\n%s",CCRED(ch, C_NRM), GET_NAME(vict),
		CCNRM(ch, C_NRM), desc);
	act(buf_temp, FALSE, ch, 0, 0, TO_CHAR);
	sprintf(buf_temp, "%s tells %s something.",GET_NAME(vict),GET_NAME(ch));
...
void rpl_user_spec (struct char_data *ch, struct char_data *vict, char *desc)
{
    ...
    if (!strncasecmp(desc, "TEST", 4)){ rpl_test(ch); return; } 
