    ...
    if (town < 0) {
        ...
        for (d = descriptor_list;d ; d = d->next) {
            ...
            sprintf(buf, "%s%s%-20s%s -- %s%-30s%s\r\n", buf, 
                    GET_LEVEL(d->character) >= LVL_AMBASSADOR ? CCYEL(ch, C_NRM) : "", 
                    GET_NAME(d->character), 
                    GET_LEVEL(d->character) >= LVL_AMBASSADOR ? CCNRM(ch, C_NRM) : "", 
                    CCCYN(ch, C_NRM), home_towns[(int)GET_HOME(d->character)],
                    CCNRM(ch, C_NRM));
        }
    } else {
...
void do_stat_memory(struct char_data * ch)
{
    int    sum = 0, total = 0;
    int    i = 0, j = 0;
...
do_stat_room(struct char_data * ch,char *roomstr)
{
        ...
        for (aff = rm->affects; aff; aff = aff->next) {
            if (aff->type == RM_AFF_FLAGS) {
                ...
                sprintf(buf, "%s  Room flags: %s%s%s, duration[%d]\r\n", 
                        buf, CCCYN(ch, C_NRM), buf2, CCNRM(ch, C_NRM), aff->duration);
    ...
    if (rm->search) {
        ...
        for (cur_search = rm->search; cur_search; cur_search = cur_search->next) {
...
do_stat_object(struct char_data * ch, struct obj_data * j)
{
    ...
    sprintf(buf, "VNum: [%s%5d%s], Exist: [%3d/%3d], Type: %s, SpecProc: %s\r\n",
            CCGRN(ch, C_NRM), GET_OBJ_VNUM(j), CCNRM(ch, C_NRM), 
    ...
    if ((rm = where_obj(j))) {
        ...
        strcat(buf, ", Aux: ");
        sprintf(buf, "%s%s%s%s", buf, CCGRN(ch, C_NRM),
                j->aux_obj ? j->aux_obj->short_description : "N", CCNRM(ch, C_NRM));
        strcat(buf, "\r\n");
        send_to_char(buf, ch);
    }
    sprintf(buf, "Material: [%s%s%s (%d)], Maxdamage: [%d (%d)], Damage: [%d]\r\n",
            CCYEL(ch, C_NRM), material_names[GET_OBJ_MATERIAL(j)], 
            CCNRM(ch, C_NRM), GET_OBJ_MATERIAL(j), 
    ...
    case ITEM_WAND:
    case ITEM_STAFF:
        sprintf(buf, "Level: %d, Max Charge: %d, Current Charge: %d, Spell: %s", 
                GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 2),
                spells[(int)GET_OBJ_VAL(j, 3)]);
        break;
    case ITEM_FIREWEAPON:
    case ITEM_WEAPON:
        sprintf(buf, "Spell: %s (%d), Todam: %dd%d (av %d), Damage Type: %s (%d)", 
                ((GET_OBJ_VAL(j, 0) > 0 && GET_OBJ_VAL(j, 0) < TOP_NPC_SPELL) ? 
                 spells[(int)GET_OBJ_VAL(j, 0)] : "NONE"),
                GET_OBJ_VAL(j, 0), 
                GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 2), 
                (GET_OBJ_VAL(j, 1) * (GET_OBJ_VAL(j, 2) + 1)) / 2,
                (GET_OBJ_VAL(j, 3) >= 0 && GET_OBJ_VAL(j, 3) < 19) ?
                attack_hit_text[(int)GET_OBJ_VAL(j, 3)].plural : "bunk", 
                GET_OBJ_VAL(j, 3));
        break;
    case ITEM_MISSILE:
    ...
    case ITEM_TRAP:
        ...
        break;
    case ITEM_CONTAINER:
        sprintf(buf, "Max-contains: %d, Locktype: %d, Key/Owner: %d, Corpse: %s, Killer: %d",
                GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 1), GET_OBJ_VAL(j, 2),
    ...
    case ITEM_FOUNTAIN:
        ...
        sprintf(buf, "Max-contains: %d, Contains: %d, Liquid: %s(%d), Poisoned: %s (%d)",
        ...
        /*  case ITEM_NOTE:
            ...
            break;*/
    case ITEM_KEY:
        sprintf(buf, "Keytype: %d, Rentable: %s, Car Number: %d", GET_OBJ_VAL(j, 0), 
                YESNO(GET_OBJ_VAL(j, 1)), GET_OBJ_VAL(j, 2));
        break;
    case ITEM_FOOD:
        sprintf(buf, "Makes full: %d, Value 1: %d, Spell : %s(%d), Poisoned: %d",
                GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 1),
                spells[(int)GET_OBJ_VAL(j, 2)], GET_OBJ_VAL(j, 2), 
                GET_OBJ_VAL(j, 3));
        break;
    case ITEM_HOLY_SYMB:
        sprintf(buf, "Alignment: %s(%d), Class: %s(%d), Min Level: %d, Max Level: %d ", 
    ...
    case ITEM_ENGINE:
        ...
        sprintf(buf, "Max_Energy: %d, Cur_Energy: %d, Run_State: %s, Consume_rate: %d", 
                GET_OBJ_VAL(j, 0), GET_OBJ_VAL(j, 1), buf2, 
                GET_OBJ_VAL(j, 3));
        break;
    case ITEM_BOMB:
    ...
    case ITEM_CLIP:
        sprintf(buf, "Values 0-3: %s:[%d] %s:[%d] %s:[%d] %s:[%s (%d)]",
                ...
                gun_types[GET_OBJ_VAL(j, 3)] : "ERROR",
                GET_OBJ_VAL(j, 3));
        break;
    case ITEM_INTERFACE:
    ...
    if (GET_OBJ_SIGIL_IDNUM(j)) {
        ...
        sprintf(buf2, "Warding Sigil: %s (%d), level %d.\r\n", 
                get_name_by_id(GET_OBJ_SIGIL_IDNUM(j)), GET_OBJ_SIGIL_IDNUM(j), GET_OBJ_SIGIL_LEVEL(j));
        send_to_char(buf2, ch);
    }
}
...
do_stat_character(struct char_data * ch, struct char_data * k)
{
    ...
    case SEX_FEMALE:     strcpy(outbuf, "FEMALE");        break;
    default:             strcpy(outbuf, "ILLEGAL-SEX!!"); break;
    }
...
/* Race, Class */ 
    ...
    if (!IS_NPC(k))
        ...
        sprintf(buf, "Lev: [%s%2d%s], XP: [%s%7d%s/%s%d%s] %s(%s%3d p%s)%s, Align: [%4d]\r\n",
                CCYEL(ch, C_NRM), GET_LEVEL(k), CCNRM(ch, C_NRM),
                CCYEL(ch, C_NRM), GET_EXP(k), CCNRM(ch, C_NRM),
                CCCYN(ch, C_NRM), rexp,
                CCNRM(ch, C_NRM), CCRED(ch, C_NRM), CCNRM(ch, C_NRM),
                GET_EXP(k) > 10000000 ?
                (((unsigned int) GET_EXP(k)) / MAX(1, rexp / 100)) :
                (((unsigned int) (GET_EXP(k) * 100)) / MAX(1, rexp)),
    ...
    if (!IS_NPC(k)) {
        ...
        sprintf(buf, 
                ...
                ((k->player.time.played / 3600) % 60), GET_AGE(k)); //age(k).year);
        strcat(outbuf, buf);

        sprintf(buf, 
                "Hometown:[%s%s%s (%d)], Loadroom: [%d/%d] %s, Clan: %s%s%s\r\n", 
                CCCYN(ch, C_NRM), home_town_abbrevs[(int)GET_HOME(k)], CCNRM(ch, C_NRM),
                GET_HOLD_HOME(k), k->player_specials->saved.load_room,
                GET_HOLD_LOADROOM(k), 
                ONOFF(PLR_FLAGGED(k, PLR_LOADROOM)),
                CCCYN(ch, C_NRM), real_clan(GET_CLAN(k)) ?
                real_clan(GET_CLAN(k))->name : "NONE",
                CCNRM(ch, C_NRM)); 
        strcat(outbuf, buf);

        sprintf(buf, "PRAC[%d] (%d/prac) (%.2f/level), Life[%d], Qpoints[%d/%d]\r\n",
                GET_PRACTICES(k),  skill_gain(k, FALSE), prac_gain(k, FALSE),
                GET_LIFE_POINTS(k), GET_QUEST_POINTS(k), GET_QUEST_ALLOWANCE(k));
        strcat(outbuf, buf);    

        sprintf(buf, "   %sMobKills:%s [ %4d ], %sPKills:%s [ %4d ], %sDeaths:%s [ %4d ]\r\n",
                CCYEL(ch, C_NRM), CCNRM(ch, C_NRM), GET_MOBKILLS(k), 
                CCRED(ch, C_NRM), CCNRM(ch, C_NRM), GET_PKILLS(k), 
                CCGRN(ch, C_NRM), CCNRM(ch, C_NRM), GET_PC_DEATHS(k));
        strcat(outbuf, buf);    
    }
    ...
    strcat(outbuf, buf);    
        if(k->in_room || !IS_NPC(k)) { // Real Mob/Char
                ...
                sprintf(buf, "Hit p.:[%s%dd%d+%d (%d)%s]  Mana p.:[%s%d%s]  Move p.:[%s%d%s]\r\n",
                        ...
                        CCGRN(ch, C_NRM), GET_MAX_MANA(k), CCNRM(ch, C_NRM),
    ...
    if (IS_NPC(k))
        sprintf(buf, "Gold:[%8d], Cash:[%8d], (Total: %d)\r\n",
                GET_GOLD(k), GET_CASH(k),
                GET_GOLD(k) + GET_CASH(k));
    else
        sprintf(buf, "Au:[%8d], Bank:[%9d], Cash:[%8d], Enet:[%9d], (Total: %d)\r\n",
    ...
    if (IS_MOB(k)) {
        sprintf(buf, "Mob Spec: %s, NPC Dam: %dd%d, Morale: %d, Lair: %d, Ldr: %d\r\n",
                (k->mob_specials.shared->func ? 
                 ((i = find_spec_index_ptr(k->mob_specials.shared->func)) < 0 ? 
                  "Exists" :
                  spec_list[i].tag) : "None"),
                k->mob_specials.shared->damnodice, 
                k->mob_specials.shared->damsizedice,
                k->mob_specials.shared->morale,
                k->mob_specials.shared->lair,
                k->mob_specials.shared->leader);
        strcat(outbuf, buf);
    
        if (MOB_SHARED(k)->move_buf)
            sprintf(outbuf, "%sMove_buf: '%s'.\r\n", outbuf, MOB_SHARED(k)->move_buf);
        if (k->mob_specials.mug) {
            sprintf(buf, "MUGGING:  victim idnum: %d, obj vnum: %d, timer: %d\r\n",
...
void do_stat_ticl(struct char_data *ch, int vnum)
{
...
ACMD(do_shutdown)
{
    ...
    extern int circle_shutdown, circle_reboot, mini_mud, 
        shutdown_idnum;
    ...
    if (!*arg) {
        ...
        House_save_all(TRUE);
        autosave_zones(ZONE_RESETSAVE);
        send_to_all("As you stand amazed, you see the sun swell and fill the sky,\r\n"
                    "and you are overtaken by an intense heat as the particles\r\n"
                    "of your body are mixed with the rest of the cosmos by the\r\n"
                    "power of a blinding supernova explosion.\r\n\r\n"
...
ACMD(do_oload)
{
    ...
    sprintf(buf, "(GC) %s loaded %s at %d.", GET_NAME(ch), obj->short_description, ch->in_room->number);
    slog(buf);
...
ACMD(do_pload)
{
    ...
    if (vict) {
        ...
        act("$n does something suspicious and alters reality.", TRUE, ch, 0, 0, TO_ROOM);
        act("$N causes $p to appear in your hands.", TRUE, vict, obj, ch, TO_CHAR);
    } else {
        act("$n does something suspicious and alters reality.", TRUE, ch, 0, 0, TO_ROOM);
        act("You create $p.", FALSE, ch, obj, 0, TO_CHAR);
        obj_to_char(obj, ch);

        sprintf(buf, "(GC) %s ploaded %s on %s.", GET_NAME(ch), obj->short_description, vict ? GET_NAME(vict) : GET_NAME(ch));
        slog(buf);
    }
}
...
ACMD(do_restore)
{
    ...
    if (!*buf)
        ...
        if ((GET_LEVEL(ch) >= LVL_GRGOD) && (GET_LEVEL(vict) >= LVL_AMBASSADOR)) {
        ...
        send_to_char(OK, ch);
        act("You have been fully healed by $N!", FALSE, vict, 0, ch, TO_CHAR);
        sprintf(buf, "%s has been restored by %s.", GET_NAME(vict), GET_NAME(ch));
...
perform_remort_invis(struct char_data *ch, int level)
{
    ...
    if (FIGHTING(ch) && level>GET_LEVEL(FIGHTING(ch)) && !IS_NPC(FIGHTING(ch))) {
        act("You cannot go invis over $N while fighting $M.", 
...
ACMD(do_invis)
{
    ...
    if (!*arg) {
        if (IS_REMORT(ch) && GET_LEVEL(ch) < LVL_AMBASSADOR) {
            ...
            level = (GET_LEVEL(ch) > LVL_LUCIFER ? LVL_LUCIFER : GET_LEVEL(ch));
...
ACMD(do_wiznet)
{
    ...
    if (!*argument) {
        if (subcmd == SCMD_IMMCHAT)
            send_to_char("Usage: imm <text> | #<level> <text> | *<emotetext> |\r\n "
                         "       imm @<level> *<emotetext> | imm @\r\n", ch);
        else
            send_to_char("Usage: wiz <text> | #<level> <text> | *<emotetext> |\r\n "
    ...
    case '@':
        for (d = descriptor_list; d; d = d->next) {
            if (!d->connected && GET_LEVEL(d->character) >= 
        ...
        any = FALSE;
        for (d = descriptor_list; d; d = d->next) {
            if (!d->connected && GET_LEVEL(d->character) >= LVL_AMBASSADOR &&
                ((subcmd == SCMD_IMMCHAT && 
                  PRF2_FLAGGED(d->character, PRF2_NOIMMCHAT)) ||
                 (subcmd == SCMD_WIZNET && 
                  PRF_FLAGGED(d->character, PRF_NOWIZ))) &&
...
ACMD(do_zreset)
{
    ...
    if (zone) {
        ...
        sprintf(buf, "(GC) %s %sreset zone %d (%s)", GET_NAME(ch), 
                subcmd == SCMD_OLC ? "olc-" : "", zone->number, 
                zone->name);
    ...
    if (!*arg)
        ...
        case SCMD_SQUELCH:
            ...
            sprintf(buf, "(GC) Squelch %s for %s by %s.", ONOFF(result),
                    GET_NAME(vict), GET_NAME(ch));
            mudlog(buf, BRF, MAX(GET_INVIS_LEV(ch), GET_INVIS_LEV(vict)), TRUE);
            strcat(buf, "\r\n");
            send_to_char(buf, ch);
            break;
        case SCMD_FREEZE:
...
void 
print_zone_to_buf(struct char_data *ch, char *bufptr, 
                  struct zone_data *zone)
{
    sprintf(bufptr, "%s%s%s%3d%s %s%-30.30s%s Age:%s%3d%s;Reset:%s%3d%s(%s%1d%s);Top: %s%5d%s;Own:%s%4d%s%s%s\r\n",
            bufptr, 
            CCBLD(ch, C_CMP), CCYEL(ch, C_NRM), zone->number, CCNRM(ch, C_CMP), 
            CCCYN(ch, C_NRM), zone->name, CCNRM(ch, C_NRM),
            CCGRN(ch, C_NRM), zone->age, CCNRM(ch, C_NRM), 
            CCGRN(ch, C_NRM), zone->lifespan, CCNRM(ch, C_NRM),
            CCRED(ch, C_NRM), zone->reset_mode, CCNRM(ch, C_NRM),
            CCGRN(ch, C_NRM), zone->top, CCNRM(ch, C_NRM),
            CCYEL(ch, C_NRM), zone->owner_idnum, 
            CCRED(ch, C_NRM), (olc_lock || IS_SET(zone->flags, ZONE_LOCKED)) ?"L":"",
            CCNRM(ch, C_NRM));
}
...
show_player(CHAR *ch, char * value)
{
    ...
    sprintf(buf,
        ...
        if(IS_SET(vbuf.player_specials_saved.plr2_bits,PLR2_BURIED))
                sprintf( buf, "%s%s%s is BURIED!%s\r\n", buf, CCGRN( ch, C_NRM ),vbuf.name, CCNRM( ch, C_NRM ) );
...
show_zoneusage(CHAR *ch, char *value)
{
    ...
    for (zone = zone_table; zone; zone = zone->next) {
        if ((i >= 0 && i != zone->number) ||
            (i == -2 && !zone->num_players))
            continue;
        sprintf(buf, "%s[%s%3d%s] %s%-30s%s %s[%s%6d%s]%s acc, [%3d] idle  Owner: %s%s%s\r\n",
                buf, CCYEL(ch, C_NRM), zone->number, CCNRM(ch, C_NRM),
                CCCYN(ch, C_NRM), zone->name,
                CCNRM(ch, C_NRM), CCGRN(ch, C_NRM), CCNRM(ch, C_NRM),
                zone->enter_count,  CCGRN(ch, C_NRM), CCNRM(ch, C_NRM),
                zone->idle_time, CCYEL(ch, C_NRM),
...
show_topzones(CHAR *ch, char *value)
{
    ...
    if ( !( topzones = ( struct zone_data **) malloc(num_zones * sizeof(struct zone_data *)) ) ) {
    ...
    for (i = 0; i < num_zones; i++) 
        sprintf(buf, "%s%2d.[%s%3d%s] %s%-30s%s %s[%s%6d%s]%s accesses.  Owner: %s%s%s\r\n",
                buf, i, CCYEL(ch, C_NRM), topzones[i]->number, 
                CCNRM(ch, C_NRM),
                CCCYN(ch, C_NRM), topzones[i]->name,
                CCNRM(ch, C_NRM), CCGRN(ch, C_NRM), CCNRM(ch, C_NRM),
                topzones[i]->enter_count,  CCGRN(ch, C_NRM), CCNRM(ch, C_NRM),
                CCYEL(ch, C_NRM),
...
ACMD(do_show)
{
    ...
    case 5:
        ...
        for (zone = zone_table; zone; zone = zone->next)
            for (room = zone->world; room; room = room->next)
                for (j = 0; j < NUM_OF_DIRS; j++)
                    if (room->dir_option[j] && room->dir_option[j]->to_room == NULL)
...
#define GOD_ROOMS_ZONE 12
    ...
    case 15:   // aliases
        if (!(vict = get_char_vis(ch, value)))
            ...
            act("You cannot determine what $S aliases might be.", FALSE, ch, 0,
                vict, TO_CHAR);
            act("$n has attempted to examine your aliases.", FALSE, ch, 0, vict,
                TO_VICT);
    ...
    case 33:  /* str_app */
        ...
        for (i = 0; i < 35; i++) {
            if (i > 25)
                ...
                strcpy(buf2, "");
            sprintf(buf, "%s%2d%-4s     %2d         %2d         %4d          %2d\r\n",
    ...
    case 40: /* demand_items */
        ...
        for (obj = obj_proto, i = 0; obj; obj = obj->next) {
            if (obj->shared->number >= k) {
                ...
                sprintf(buf, "%s%4d. %s[%s%5d%s] %40s%s Tot:[%4d], House:[%4d]\r\n",
                        buf, ++i, 
                        CCGRN(ch, C_NRM), CCNRM(ch, C_NRM),
                        GET_OBJ_VNUM(obj), 
                        CCGRN(ch, C_NRM), obj->short_description,
                        CCNRM(ch, C_NRM),
    ...
    case 47:  // duperooms
        ...
        strcpy(buf, " Zone  Name                          Rooms       Bytes\r\n");
        for (zone = zone_table, k = con = tot_rms = 0; zone; zone = zone->next) {
            ...
            for (room = (zone->world ? zone->world->next : NULL), num_rms = 1; 
                ...
                if (room->description) {
                    ...
                    for (tmp_room = zone->world; tmp_room; tmp_room = tmp_room->next) {
                        if (tmp_room == room)
                            break;
                        if (!tmp_room->description)
                            continue;
                        if (!strncasecmp(tmp_room->description, room->description, l)) {
    ...
    case 49: // p_index
        ...
        if (*value) {
            ...
            if (*arg) {
                k = atoi(arg);
                if (k < j) {
                    send_to_char("The top value must be greater than the first.\r\n",ch);
                    return;
                }
            }
        }
        ...
        if (*value == 't') {
            ...
            for (zone = zone_table, j = 0, found = 0; zone; zone = zone->next) {
                ...
                for (room = zone->world; room; room = room->next) {
                    for (i = 0; i < NUM_DIRS; i++) {
                        if (room->dir_option[i] && room->dir_option[i]->to_room &&
                            room->dir_option[i]->to_room->zone->number == k) {
                            if (strlen(buf) > MAX_STRING_LENGTH - 128) {
                                strcat(buf, "**OVERFLOW**\r\n");
                                break;
                            }
...
ACMD(do_set)
{
    ...
    case 12:
        if (vict->real_abils.str != 18) {
            send_to_char("Stradd is only applicable with 18 str... asshole.\r\n",
                         ch);
            return;
        }
    ...
    case 37:
        ...
        sprintf(buf, "(GC) %s %ssiteok'd %s.", GET_NAME(ch), PLR_FLAGGED(vict, PLR_SITEOK) ? "" : "UN-", GET_NAME(vict));
        slog(buf);
    ...
    case 71:
        ...
        SET_BIT(PRF_FLAGS(vict), (PRF_LOG1 * (tp & 1)) | (PRF_LOG2 * (tp & 2) >> 1));
  
        sprintf(buf, "%s's syslog is now %s.\r\n", GET_NAME(vict), logtypes[tp]);
        send_to_char(buf, ch);
        break;
    case 72:
    ...
    for (obj = obj_proto; obj && (obj->shared->vnum <= last); 
        ...
        if (obj->shared->vnum >= first) {
            sprintf(buf, "%5d. %s[%s%5d%s]%s %-36s%s %s %s\r\n", ++found,
                    ...
                    obj->short_description, CCNRM(ch, C_NRM),
                    !P_OBJ_APPROVED(obj) ? "(!aprvd)" : "", (!(obj->description)||!(*(obj->description))) ? "(nodesc)" : "");
            if ((strlen(out_list) + strlen(buf)) < MAX_STRING_LENGTH - 20)
                strcat(out_list, buf);
            else if (strlen(out_list) < (MAX_STRING_LENGTH - 20)) {
                strcat(out_list, "**OVERFLOW**\r\n");
...
ACMD(do_mudwipe)
{
    ...
    if (mode == 0 || mode == 1) {
        for (zone = zone_table; zone; zone = zone->next)
            for (room = zone->world; room; room = room->next) {
                if (ROOM_FLAGGED(room, ROOM_GODROOM | ROOM_CLAN_HOUSE | ROOM_HOUSE))
                    continue;
...
#ifdef TRACK_OBJS      
            sprintf(buf, "Object: %s [%s] %s", obj->short_description, obj->obj_flags.tracker.string, ctime(&obj->obj_flags.tracker.lost_time));
#else
...
do_show_mobiles(struct char_data *ch, char *value, char *arg)
{
    ...
    case 4: /* extreme gold */
        ...
        arg = two_arguments(arg, arg1, arg2);
        if (!*arg1) {
            send_to_char("Usage: show mob extreme <gold | cash> [multiplier]\r\n", ch);
            return;
        }
        ...
        if (is_abbrev(arg1, "gold"))
            ...
            sprintf(buf, "You must specify gold or cash.  '%s' is not valid.\r\n", arg1);
            send_to_char(buf, ch);
            return;
        }
...
do_show_objects(struct char_data *ch, char *value, char *arg)
{
            ...
            for (k = 0; k < MAX_OBJ_AFFECT; k++)
                if (obj->affected[k].location == i) {
                    sprintf(buf2, "%3d. [%5d] %s%-34s%s  (%s%3d%s)%s\r\n",
                            ...
                            CCNRM(ch, C_NRM), !OBJ_APPROVED(obj) ? "  (!appr)" : "");
    ...
    case 4: /* affect */
        ...
        if ((!(i = 1) || (j = search_block(arg,  affected_bits_desc, 0)) < 0) &&
            ...
            (!(i = 3) || (j = search_block(arg, affected3_bits_desc, 0)) < 0)) {
