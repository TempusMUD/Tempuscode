...
show_menu(struct descriptor_data *d)
{
    ...
    sprintf(buf, "%s                      %s1%s)%s Go forth into the realm.%s\r\n", buf,
	    CCYEL(d->character, C_NRM), CCRED(d->character, C_NRM),
	    CCGRN(d->character, C_NRM), CCNRM(d->character, C_NRM));
    sprintf(buf, "%s                      %s2%s)%s Enter description.%s\r\n", buf,
	    CCYEL(d->character, C_NRM), CCRED(d->character, C_NRM),
	    CCGRN(d->character, C_NRM), CCNRM(d->character, C_NRM));
    sprintf(buf, "%s                      %s3%s)%s Read the background story.%s\r\n", buf,
	    CCYEL(d->character, C_NRM), CCRED(d->character, C_NRM),
	    CCGRN(d->character, C_NRM), CCNRM(d->character, C_NRM));
    sprintf(buf, "%s                      %s4%s)%s Change password.%s\r\n", buf,
	    CCYEL(d->character, C_NRM), CCRED(d->character, C_NRM),
	    CCGRN(d->character, C_NRM), CCNRM(d->character, C_NRM));
    sprintf(buf, "%s                      %s5%s)%s Delete this character.%s\r\n", buf,
	    CCYEL(d->character, C_NRM), CCRED(d->character, C_NRM),
	    CCGRN(d->character, C_NRM), CCNRM(d->character, C_NRM));
...
show_char_class_menu_past(struct descriptor_data *d)
{
    ...
    sprintf(buf, "%s                %sThief%s     --  Stealthy Rogue\r\n", 
	    buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM));
    sprintf(buf, "%s                %sMonk%s      --  Philosophical Warrior\r\n", 
	    buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM));
...
show_char_class_menu_future(struct descriptor_data *d)
{
    ...
    sprintf(buf, "%s                %sMercenary%s   --  Gun for Hire\r\n", 
	    buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM));
    sprintf(buf, "%s                %sPhysic%s      --  Controller of Forces\r\n", 
	    buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM));
    sprintf(buf, "%s                %sHoodlum%s     --  The Lowdown Street Punk\r\n", 
	    buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM));
    sprintf(buf, "%s                %sMonk%s        --  Philosophical Warrior\r\n", 
	    buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM));
...
show_past_home_menu(struct descriptor_data *d)
{
    ...
    sprintf(buf, "%s    %sIstan%s            -- Northern Frontier City.  Population [%d]\r\n", buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM),
	    population_record[HOME_ISTAN]);
    sprintf(buf, "%s    %sElven Village%s    -- Elves Only.              Population [%d]\r\n", buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM),
	    population_record[HOME_ELVEN_VILLAGE]);
    sprintf(buf, "%s    %sSolace Cove%s      -- Western Seacost.         Population [%d]\r\n", buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM),
	    population_record[HOME_SOLACE_COVE]);
    sprintf(buf, "%s    %sModrian%s          -- Primary City.            Population [%d]\r\n", buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM),
	    population_record[HOME_MODRIAN]);
    sprintf(buf, "%s    %sTower of Modrian%s -- Zone for New Players.    Population [%d]\r\n", buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM),
	    population_record[HOME_NEWBIE_TOWER]);
    sprintf(buf, "%s    %sSkullport%s        -- City of the Underdark.   Population [%d]\r\n", buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM),
...
show_future_home_menu(struct descriptor_data *d)
{
    ...
    sprintf(buf, "%s    %sZul'Dane%s                -- Orcish Industrial City.    Population [%d]\r\n", buf, CCGRN(d->character, C_NRM), CCCYN(d->character, C_NRM),
...
show_race_restrict_past(struct descriptor_data *d)
{
    ...
    strcat(buf, "           HALF-ORC      can't be - cleric, mage, knight, monk\r\n");
