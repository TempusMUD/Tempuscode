        ...
        if (ROOM_FLAGGED(ch->in_room, ROOM_SOUNDPROOF))
            ...
            if (subcmd == SCMD_QSAY) {
                send_to_char(ch, "%s%sYou quest-say,%s '%s'\r\n", CCYEL(ch,
                        C_NRM), CCBLD(ch, C_SPR), CCNRM(ch, C_SPR), argument);
            } else {
                send_to_char(ch, "%s%s%s\r\n",
                    CCYEL_BLD(ch, C_NRM), argument, CCNRM(ch, C_NRM));
            }
        }
        ...
        if (subcmd == SCMD_QSAY) {
            for (i = descriptor_list; i; i = i->next)
                if (STATE(i) == CXN_PLAYING && i != ch->desc &&
                    ...
                    send_to_char(i->creature, "%s%s quest-says,%s '%s'\r\n",
                        ...
                        PERS(ch, i->creature),
                        CCNRM(i->creature, C_SPR), argument);
                }
        } else {
            strcpy(buf, argument);
            ...
            for (i = descriptor_list; i; i = i->next)
                if (STATE(i) == CXN_PLAYING && i != ch->desc &&
                    ...
                    !PLR_FLAGGED(i->creature,
                        PLR_MAILING | PLR_WRITING | PLR_OLC)) {
                    send_to_char(i->creature, "%s", CCYEL_BLD(i->creature,
                            C_NRM));
