...
ACMD(do_say)
{
    struct char_data *i = NULL, *vict = NULL;
    char name[MAX_INPUT_LENGTH], buf3[MAX_INPUT_LENGTH], buf[MAX_STRING_LENGTH];
	...
	case SCMD_EXPOSTULATE:
	    send_to_char("You expostulate at length about your latest theories.\r\n", ch);
	    act("$n expostulates at length about $s latest theories.", 
		FALSE, ch, 0, 0, TO_ROOM);
	    break;
	case SCMD_RAMBLE:
	    send_to_char("You ramble endlessly, much to your own delight.\r\n",ch);
	    act("$n rambles endlessly, much to your chagrin.",
		FALSE, ch, 0, 0, TO_ROOM);
	    break;
	case SCMD_BABBLE:
	    send_to_char("You babble incoherently for quite some time.\r\n", ch);
	    act("$n babbles completely incoherently for a while.", 
		FALSE, ch, 0, 0, TO_ROOM);
	    break;
	case SCMD_MURMUR:
	...
	if (o) {
		...
		if (ABS_EXIT(was_in, j) && ABS_EXIT(was_in, j)->to_room &&
		    ...
		    !IS_SET(ABS_EXIT(was_in, j)->exit_info, EX_ISDOOR | EX_CLOSED)) {
		    char_from_room(ch);
	...
	if (!*name)
		...
		if (!AWAKE(i) || i == ch || 
		    (i->desc && i->desc->showstr_point && 
		     !PRF2_FLAGGED(i, PRF2_LIGHT_READ)) ||
		    PLR_FLAGGED(i, PLR_OLC | PLR_WRITING | PLR_MAILING))
		    continue;
		strcpy(buf, PERS(ch, i));
		strcpy(buf2, CAP(buf));
		if (i == vict)
		    strcpy(buf3, "you");
		else if (vict == ch) {
		    ...
		    strcpy(buf3, PERS(vict, i));
		sprintf(buf, "%s%s%s%s says to %s,%s %s'%s'%s\r\n", 
			recurs_say ? "(remote) " : "", CCBLD(i, C_NRM), 
			CCBLU(i, C_SPR), buf2, buf3, CCNRM(i, C_SPR), CCCYN(i, C_NRM),
			argument, CCNRM(i, C_NRM));
		send_to_char(buf, i);
	    }
	    if (!recurs_say) {
		if (PRF_FLAGGED(ch, PRF_NOREPEAT))
		    ...
		    sprintf(buf, "%s%sYou say to %s,%s %s'%s'%s\r\n", CCBLD(ch, C_NRM), 
			    CCBLU(ch, C_NRM), GET_DISGUISED_NAME(ch, vict), 
			    CCNRM(ch, C_NRM), 
	...
	if (!AWAKE(i) || i == ch || 
	    (i->desc && i->desc->showstr_point && 
	     !PRF2_FLAGGED(i, PRF2_LIGHT_READ)) ||
	    PLR_FLAGGED(i, PLR_OLC | PLR_MAILING | PLR_WRITING))
	    continue;
	strcpy(buf, PERS(ch, i));
	strcpy(buf2, CAP(buf));
	sprintf(buf, "%s%s%s%s %s,%s %s'%s'%s\r\n", recurs_say ? "(remote) " : "",
		CCBLD(i, C_NRM), 
		CCBLU(i, C_SPR), buf2, 
		(subcmd == SCMD_UTTER ? "utters" :
		 subcmd == SCMD_EXPOSTULATE ? "expostulates" :
		 subcmd == SCMD_RAMBLE ? "rambles" :
		 subcmd == SCMD_BELLOW ? "bellows" :
		 subcmd == SCMD_MURMUR ? "murmurs" :
		 subcmd == SCMD_INTONE ? "intones" :
		 subcmd == SCMD_YELL   ? "yells" :
		 subcmd == SCMD_BABBLE ? "babbles" : "says"),
		CCNRM(i, C_SPR), CCCYN(i, C_NRM),
		argument, CCNRM(i, C_NRM));
	send_to_char(buf, i);
    }
    if (!recurs_say) {
...
ACMD(do_gsay)
{
	...
	for (f = k->followers; f; f = f->next)
		...
		sprintf(buf, "%s$n tells the group,%s '%s'%s", CCGRN(f->follower, C_NRM),
			CCYEL(f->follower, C_NRM), argument, CCNRM(f->follower, C_NRM));
...
ACMD(do_gen_comm)
{
	...
	if (COLOR_LEV(ch) >= C_NRM)
	    sprintf(buf1, "%sYou %s,%s%s '%s'%s", color_on1, com_msgs[subcmd][1],
		    KNRM, color_on2, argument, KNRM);
	else
	...
	if (!IS_MOB(ch) && GET_LEVEL(ch) < LVL_GRGOD && 
	    ...
	    switch (number(0, 2)) {
	    case 0:
		send_to_char("Unless you like being muted, use SPEW for profanity.\r\n", ch);
		break;
	    case 1:
		send_to_char("Why don't you keep that smut on the spew channel, eh?\r\n", ch);
		break;
	    case 2:
		send_to_char("I don't think everyone wants to hear that.  SPEW it!\r\n", ch);
		break;
	    }
	    sprintf(buf, "%s warned for nasty public communication.", GET_NAME(ch));
	    slog(buf);
	}
    }

    sprintf(buf, "$n %ss, '%s'", com_msgs[subcmd][1], argument);
    sprintf(buf2, "$n %ss,%s%s '%s'", com_msgs[subcmd][1], KNRM, color_on2, argument);
...
ACMD(do_qcomm)
{
	...
	if (subcmd == SCMD_QSAY) {
		...
		if (!i->connected && i != ch->desc &&
		    ...
		    !ROOM_FLAGGED(i->character->in_room, ROOM_SOUNDPROOF) && 
		    (!i->showstr_point || PRF2_FLAGGED(ch, PRF2_LIGHT_READ)) &&
		    !PLR_FLAGGED(i->character, PLR_MAILING|PLR_WRITING|PLR_OLC)) {
		    strcpy(buf, CCYEL_BLD(i->character, C_SPR));
		    sprintf(buf2, "%s quest-says,%s '%s'\r\n", PERS(ch, i->character), 
			    CCNRM(i->character, C_SPR), argument);
		    strcat(buf, CAP(buf2));
	...
	if (subcmd == SCMD_CLAN_SAY) {
		...
		if (!i->connected && i != ch->desc && i->character && 
		    ...
		    sprintf(buf2, "%s clan-says, %s'%s'\r\n", PERS(ch, i->character), 
			    CCNRM(i->character, C_NRM), argument);
		    strcat(buf, CAP(buf2));
		    send_to_char(buf, i->character);
		}
	} else {
		...
		if (!i->connected && i != ch->desc && i->character && 
		    ...
		    (!i->showstr_point || PRF2_FLAGGED(ch, PRF2_LIGHT_READ)) &&
		    !PLR_FLAGGED(i->character, PLR_MAILING | PLR_WRITING | PLR_OLC)) {
		    sprintf(buf, "$n%s %s", CCNRM(i->character, C_NRM), argument);
