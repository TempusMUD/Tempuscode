...
explode_sigil(CHAR *ch, OBJ *obj)
{
	...
	act("$p feels rather warm to the touch, and shudders violently.", FALSE, ch, obj, 0, TO_CHAR);
	return 0;
    }
	    ...
	    struct obj_data * cont, int display)
{
	...
	if (CAR_CLOSED(cont))
	    ...
	    act("$p is tossed into the car by $N.", FALSE, 0, obj, ch, TO_ROOM);
	}
    } else {
...
ACMD(do_put)
{
	...
	generic_find(arg2, FIND_OBJ_INV | FIND_OBJ_EQUIP_CONT | FIND_OBJ_ROOM, ch, &tmp_char, &cont);
	if (!cont) {
...
int 
get_check_money(struct char_data * ch, struct obj_data * obj, int display)
{
...
perform_get_from_container(struct char_data * ch, struct obj_data * obj,
			   struct obj_data * cont, int mode, int display)
{

    if (mode == FIND_OBJ_INV || mode == FIND_OBJ_EQUIP_CONT ||
	can_take_obj(ch, obj)) {
	if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch))
	    ...
	    obj_from_obj(obj);
	    obj_to_char(obj, ch);
	    if (display == TRUE || IS_OBJ_TYPE(obj, ITEM_MONEY)) {
	    ...
	    if (get_check_money(ch, obj, TRUE))
		return 0;
	    return 1;
	}
	return 0;
		   ...
		   char *arg, int mode)
{
    ...
    int obj_dotmode, found = 0, counter = 0;
    int sigil_trip = 0;
	...
	if (!(obj = get_obj_in_list_all(ch, arg, cont->contains)) ||
	    ...
	    // please do not attempt to trip a sigil on a picked up MONEY item.
	    // it will have been extracted in perform_get_from_room...
	    if (IS_OBJ_TYPE(obj, ITEM_MONEY)) 
		sigil_trip = 1;
	
	    if (perform_get_from_container(ch, obj, cont, mode, TRUE) && !sigil_trip &&
		GET_OBJ_SIGIL_IDNUM(obj) && GET_OBJ_SIGIL_IDNUM(obj) != GET_IDNUM(ch)) {
	    
		explode_sigil(ch, obj);
	    }
	}
	return;
    } 
	...
	for (obj = cont->contains, counter = 1, found = 0; 
	     obj; obj = next_obj, sigil_trip = 0) {
	    next_obj = obj->next_content;
	    ...
	    if (IS_CARRYING_N(ch) >= CAN_CARRY_N(ch)) {
		send_to_char("Your arms are overloaded.\r\n", ch);
		return;
	    }
	    ...
	    if (IS_OBJ_TYPE(obj, ITEM_MONEY)) {
		perform_get_from_container(ch, obj, cont, mode, FALSE);
		counter = 1;
		continue;
	    } else if (!perform_get_from_container(ch, obj, cont, mode, FALSE)) {
		counter = 1;
		continue;
	    } 
	    ...
	    if (next_obj && next_obj->short_description == obj->short_description && !sigil_trip &&
		...
		// code to blow up the sigil and return
		if (sigil_trip) {
	...
	if (!(obj = get_obj_in_list_vis(ch, arg, cont->contains))) {
	    sprintf(buf, "You don't see any %ss in %s.\r\n", 
		    arg, cont->short_description);
	    send_to_char(buf, ch);
	    return;
	}
    ...
    if (!found) {
	if (obj_dotmode == FIND_ALL)
	    send_to_char("There doesn't seem to be anything in it.\r\n", ch);
	else {
	    sprintf(buf, "You don't see any %ss in %s.\r\n", 
		    arg, cont->short_description);
	    send_to_char(buf, ch);
		      ...
		      struct obj_data * obj, int display)
{
	...
	extract_obj(obj);
	sprintf(buf, "%s got the Quad Damage at %d.",
	...
	if (!ROOM_FLAGGED(ch->in_room, ROOM_SOUNDPROOF)) {
		...
		for (zone = zone_table; zone; zone = zone->next) {
			...
			((zone->plane <= MAX_PRIME_PLANE && 
			  ch->in_room->zone->plane <= MAX_PRIME_PLANE) ||
			 ch->in_room->zone->plane == zone->plane) &&
			!ZONE_FLAGGED(zone, ZONE_ISOLATED | ZONE_SOUNDPROOF)) {
			for (room = zone->world; room; room = room->next) {
			    if (room->people &&
				!ROOM_FLAGGED(room, ROOM_SOUNDPROOF))
				send_to_room("You hear a screaming roar in the distance.\r\n", room);
			}
		    }
		}
	    }
    ...
    get_check_money(ch, obj, TRUE);
    return 1;
}

int
perform_get_money_from_room(struct char_data *ch, int mode)
{
    struct obj_data *next_obj, *obj = NULL;
    int total_g = 0, total_c = 0;
  
    for (obj = ch->in_room->contents; obj; obj = next_obj) {
	next_obj = obj->next_content;
	if (GET_OBJ_TYPE(obj) == ITEM_MONEY && CAN_SEE_OBJ(ch, obj) &&
	    (mode == 2 || mode == GET_OBJ_VAL(obj, 1))) {
	    obj_from_room(obj);
	    obj_to_char(obj, ch);
	    if (GET_OBJ_VAL(obj, 1)) // credits
		total_c += GET_OBJ_VAL(obj, 0);
	    else
		total_g += GET_OBJ_VAL(obj, 0);
	    get_check_money(ch, obj, FALSE);
	}
    }
  
    if (total_c || total_g){
	send_to_char("You get all the money in the room.\r\n", ch);
	act("$n gets all the money in the room.", TRUE, ch, 0, 0, TO_ROOM);
	if (total_g) {
...
{
    struct obj_data *obj, *next_obj;
    int dotmode, found = 0, money_found = 0;
    int counter = 0, end = 0;
    int sigil_trip = 0;
	...
	if (!(obj = get_obj_in_list_vis(ch, arg, ch->in_room->contents))) {
	    ...
	    if (perform_get_from_room(ch, obj, TRUE) && !sigil_trip &&
		GET_OBJ_SIGIL_IDNUM(obj) && GET_OBJ_SIGIL_IDNUM(obj) != GET_IDNUM(ch)) {
	...
	found = perform_get_money_from_room(ch, 2);

	for (obj = ch->in_room->contents, counter = 1; 
	     obj; obj = next_obj, end = FALSE, sigil_trip = 0) {
	    next_obj = obj->next_content;

	    if (!CAN_SEE_OBJ(ch, obj) || !perform_get_from_room(ch, obj, FALSE)) {
		if (counter == 1)
		    continue;
		else {
		    end = TRUE;
	...
	if (!str_cmp(arg, "gold") || !str_cmp(arg, "coins")) {
	    money_found = TRUE;
	    perform_get_money_from_room(ch, 0);
	}
	else if (!str_cmp(arg, "money")) {
	    money_found = TRUE;
	    perform_get_money_from_room(ch, 2);
	}
	else if (!str_cmp(arg, "credits") || !str_cmp(arg, "cash")) {
	    money_found = TRUE;
	    perform_get_money_from_room(ch, 1);
	}
		     ...
		     byte mode, struct room_data *RDR)
{
	...
	if (mode != SCMD_JUNK) {
	    ...
	    if (mode == SCMD_DONATE) {
		send_to_char("You throw some cash into the air where it disappears in a puff of smoke!\r\n", ch);
		act("$n throws some cash into the air where it disappears in a puff of smoke!",
		    FALSE, ch, 0, 0, TO_ROOM);
		obj_to_room(obj, RDR);
		act("$p suddenly appears in a puff of orange smoke!", 0, 0, obj, 0, TO_ROOM);
	    } else {
	    ...
	    sprintf(buf, "$n drops %s which disappears in a puff of smoke!",
		    money_desc(amount, 1));
	    act(buf, FALSE, ch, 0, 0, TO_ROOM);
	    send_to_char("You drop some cash which disappears in a puff of smoke!\r\n", ch);
	}
	GET_CASH(ch) -= amount;
	if (GET_LEVEL(ch) >= LVL_AMBASSADOR && GET_LEVEL(ch) < LVL_GOD) {
	    sprintf(buf, "MONEY: %s has dropped %d credits at %d.", GET_NAME(ch),
		    amount, ch->in_room->number);
	    slog(buf);
	}
    }
...
ACCMD(do_drop)
{
	...
	case SCMD_DONATE:
	    ...
	    case 3:
	    case 4:
	    case 5: RDR = real_room(donation_room_1); break;	 /* Modrian */
	    case 6:
	    case 7: RDR = real_room(donation_room_3); break;     /* New Thalos */
	    case 8:
	    case 9: RDR = real_room(donation_room_istan); break; /* Istan */
	    case 10:
	    case 11: RDR = real_room(donation_room_solace); break; /* Solace Cove */
	    case 12:
	    case 13:
	    case 14: RDR = real_room(donation_room_skullport_common); break; /* sk */
	    case 15: RDR = real_room(donation_room_skullport_dwarven); break;
	    case 16:
	    case 17:
	    case 18: RDR = real_room(donation_room_2); break;
	    }
	     ...
	     struct obj_data * obj, int display)
{
	...
	if (IS_BOMB(obj)) {
	    if ((isname("bomb", obj->name) || isname("grenade", obj->name) ||
		 isname("explosives", obj->name) || isname("explosive", obj->name))&&
		...
		if (!FIGHTING(vict) && 
		    ...
		    if (CAN_SEE(vict, ch)) {
			sprintf(buf, "%s %s", fname(obj->name), fname(ch->player.name));
	...
	if ((IS_CARRYING_W(vict) + IS_WEARING_W(vict)) > 
	    (CAN_CARRY_W(vict) >> 1))  // i don't want that heavy shit
...
void 
perform_plant_gold(struct char_data * ch, struct char_data * vict,
		   int amount)
{
    if (amount <= 0) {
	send_to_char("Heh heh heh ... we are jolly funny today, eh?\r\n", ch);
    ...
    send_to_char(OK, ch);
    if ((GET_SKILL(ch, SKILL_PLANT) + GET_DEX(ch)) < (number(0, 83) + GET_WIS(vict)) ||
	IS_AFFECTED_2(vict, AFF2_TRUE_SEEING)) {
...
void 
perform_give_credits(struct char_data * ch, struct char_data * vict,
		     int amount)
{
    if (amount <= 0) {
	send_to_char("Heh heh heh ... we are jolly funny today, eh?\r\n", ch);
...
void 
perform_plant_credits(struct char_data * ch, struct char_data * vict,
		      int amount)
{
    if (amount <= 0) {
	send_to_char("Heh heh heh ... we are jolly funny today, eh?\r\n", ch);
...
ACMD(do_drink)
{
	...
	if (!number(0, 1))
	    act("$n chokes and utters some strange sounds.",FALSE,ch,0,0,TO_ROOM);
	else
...
ACMD(do_eat)
{
    ...
    if ( ( GET_LEVEL( ch ) >= LVL_AMBASSADOR || ROOM_FLAGGED( ch->in_room, ROOM_ARENA ) ) && !IS_MOB( ch ) )
...
ACMD(do_pour)
{
    ...
    if (GET_OBJ_VAL(from_obj, 0) == -1) {
	act("You cannot seem to pour from $p.", FALSE, ch, from_obj, 0, TO_CHAR);
	return;
    }
    if (subcmd == SCMD_POUR) {
...
perform_wear(struct char_data * ch, struct obj_data * obj, int where)
{
	...
	if (GET_EQ(ch, WEAR_WIELD)) {
		...
		act("You are currently wielding $p and holding $P.",
		    FALSE, ch, GET_EQ(ch, WEAR_WIELD), GET_EQ(ch, WEAR_HOLD), TO_CHAR);
		return 0;
	    } else if (GET_EQ(ch, WEAR_SHIELD)) {
		act("You are currently wielding $p and using $P.",
		    FALSE, ch, GET_EQ(ch, WEAR_WIELD),GET_EQ(ch,WEAR_SHIELD),TO_CHAR);
		return 0;
	    }
	}
...
ACMD(do_wear)
{
	...
	for (obj = ch->carrying; obj; obj = next_obj) {
	    next_obj = obj->next_content;
	    if (INVIS_OK_OBJ(ch, obj) && (where = find_eq_pos(ch, obj, 0)) >= 0) {
		items_worn++;
...
perform_remove(struct char_data * ch, int pos)
{
	...
	act("$p: you must have this removed by a professional.",
	    FALSE, ch, obj, 0, TO_CHAR);
    else if (IS_OBJ_STAT2(obj, ITEM2_NOREMOVE) && GET_LEVEL(ch)<LVL_AMBASSADOR)
	act("$p: you cannot convince yourself to stop using it.", 
	    FALSE, ch, obj, 0, TO_CHAR);
    else {
...
ACMD(do_remove)
{
	...
	send_to_char("You cannot remove anything while generating an attraction field!\r\n", ch);
	return;
    }
	...
	if (!(obj = get_object_in_equip_all(ch, arg, ch->equipment, &i))) {
	    ...
	    act("Ack!  You cannot seem to let go of $p!",FALSE,ch, obj, 0, TO_CHAR);
	else
	    perform_remove(ch, i);
    }
}
...
prototype_obj_value(struct obj_data *obj)
{
    ...
    case ITEM_BULLET:
	if (GUN_TYPE(obj) >= 0 && GUN_TYPE(obj) < NUM_GUN_TYPES)
	    value += (gun_damage[GUN_TYPE(obj)][0] * gun_damage[GUN_TYPE(obj)][1]);
	value += (BUL_DAM_MOD(obj) * 100);
	break;

    case ITEM_ENERGY_GUN:
	value += GUN_DISCHARGE(obj) * GUN_DISCHARGE(obj) *
	    GUN_DISCHARGE(obj) * MAX_R_O_F(obj) * MAX_R_O_F(obj) * MAX_R_O_F(obj);
...
choose_material(struct obj_data *obj)
{
	...
	if (str_cmp(name, "a") && str_cmp(name, "an") && str_cmp(name, "the") &&
	    str_cmp(name, "some") && str_cmp(name, "of") && str_cmp(name, "black") &&
	    str_cmp(name, "white") && str_cmp(name, "green") && str_cmp(name, "brown")) {
	    for (i = 0; i < TOP_MATERIAL; i++)
	...
	while (*name) {
		...
		str_cmp(name, "some") && str_cmp(name, "of") && str_cmp(name, "black") &&
		str_cmp(name, "white") && str_cmp(name, "green") && str_cmp(name, "brown") &&
		str_cmp(name, "lying") && str_cmp(name, "has") && str_cmp(name, "been") &&
		str_cmp(name, "is") && str_cmp(name, "left") && str_cmp(name, "here") &&
		str_cmp(name, "set") && str_cmp(name, "lies") && str_cmp(name, "looking") &&
		str_cmp(name, "pair") && str_cmp(name, "someone") && str_cmp(name, "there") &&
		str_cmp(name, "has") && str_cmp(name, "on") && str_cmp(name, "dropped")) {
		for (i = 0; i < TOP_MATERIAL; i++)
		    if (isname(material_names[i], name))
			return (i);
	    }
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_FOOD && (isname("loaf", obj->name) ||
					   ...
					   isname("bisquit", obj->name) ||
					   isname("cake", obj->name)))
	return (MAT_BREAD);
    ...
    if (GET_OBJ_TYPE(obj) == ITEM_FOOD && (isname("fish", obj->name) ||
					   ...
					   isname("sausage", obj->name) ||
					   isname("fillet", obj->name)))
	return (MAT_MEAT_COOKED);
