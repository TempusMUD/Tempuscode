...
int explode_sigil(CHAR *ch, OBJ *obj) {
...
//
int explode_all_sigils( struct char_data *ch ) {
...
void consolidate_char_money( struct char_data *ch ) {
...
bool activate_char_quad( struct char_data *ch ) {
    ...
    for ( obj = ch->carrying; obj; obj = next_obj ) {
        ...
        if (GET_OBJ_VNUM(obj) == QUAD_VNUM) {
            ...
            extract_obj(obj);
            sprintf(buf, "%s got the Quad Damage at %d.", GET_NAME(ch), ch->in_room->number);
            slog(buf);
...
bool can_take_obj(struct char_data * ch, struct obj_data * obj, bool check_weight, bool print )
{
    
    if ( IS_CARRYING_N(ch) >= CAN_CARRY_N(ch) ) {
        sprintf( buf, "$p: you can't carry that many items." );
    } else if ( check_weight && (IS_CARRYING_W(ch) + obj->getWeight()) > CAN_CARRY_W(ch) ) {
...
int get_check_money(struct char_data * ch, struct obj_data ** obj_p, int display)
{
...
bool perform_get_from_container( struct char_data * ch, 
                                 struct obj_data * obj,
...
int get_from_container( struct char_data * ch, struct obj_data * cont, char *arg ) {
    ...
    //
        ...
        for ( obj = cont->contains; obj; obj = next_obj ) {
            ...
            if ( !next_obj || 
                 ...
                 !CAN_SEE_OBJ( ch, next_obj ) || !can_take_obj( ch, next_obj, check_weight, false ) ) {
                display = true;
            }

            if ( ! perform_get_from_container( ch, obj, cont, check_weight, display, counter ) ) {
                counter = 1;
            }
                ...
                if ( IS_OBJ_TYPE( obj, ITEM_MONEY ) )
                    ...
                    quad_found = true;
                else if ( GET_OBJ_SIGIL_IDNUM( obj ) && GET_OBJ_SIGIL_IDNUM(obj) != GET_IDNUM(ch) )
                    sigil_found = true;
        ...
        if ( !found ) {
            ...
            if ( match_name ) {
                sprintf( buf, "You didn't find anything in $P to take that looks like a '%s'", 
                         match_name );
...
bool perform_get_from_room( struct char_data * ch, 
                            struct obj_data * obj, 
                            bool display, int counter ) {
...
int get_from_room(struct char_data * ch, char *arg) {
    ...
    //
        ...
        for ( obj = ch->in_room->contents; obj; obj = next_obj ) {
            ...
            if ( !next_obj || 
                 next_obj->short_description != obj->short_description ||
                 !CAN_SEE_OBJ( ch, next_obj ) || !can_take_obj( ch, next_obj, true, false ) ) {
                display = true;
            }
        ...
        if ( !found ) {
            ...
            if ( match_name ) {
                sprintf( buf, "You didn't find anything to take that looks like a '%s'.\r\n", match_name );
                send_to_char( buf, ch );
            }
            else {
...
ACCMD(do_get)
{
    ...
    for ( cont = ch->carrying; cont; cont = cont->next_content ) {
        ...
        if ( CAN_SEE_OBJ( ch, cont ) &&
             ( !match_container_name || isname( match_container_name, cont->name ) ) ) {
