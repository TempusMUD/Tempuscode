...
show_olc_help(struct creature *ch, char *arg)
{
    ...
    case 3:
        ...
        for (i = 0; i < NUM_DOORFLAGS; i++) {
            snprintf(buf2, sizeof(buf2), "  %s         %s%s%s\r\n",
                     tmp_asciiflag(1U << i), CCCYN(ch, C_NRM),
    ...
    case 7:
        ...
        break;
    case 8:
        strcpy_s(buf, sizeof(buf), "##  LIQUID            Drunk     Hunger   Thirst\r\n");
        for (i = 0; i < NUM_LIQUID_TYPES; i++) {
            snprintf_cat(buf, sizeof(buf), "%2d  %s%-20s%s  %3d   %3d  %3d\r\n",
    ...
    case 11:
        ...
        for (i = 0; i < NUM_AFF_FLAGS; i++) {
            snprintf(buf2, sizeof(buf2), "  %s         %s%s%s\r\n", tmp_asciiflag(1U << i),
                     CCCYN(ch, C_NRM), affected_bits_desc[i], CCNRM(ch, C_NRM));
            strcat_s(buf, sizeof(buf), buf2);
        }
        page_string(ch->desc, buf);
        break;
    case 12:
        ...
        for (i = 0; i < NUM_AFF2_FLAGS; i++) {
            snprintf(buf2, sizeof(buf2), "  %s         %s%s%s\r\n", tmp_asciiflag(1U << i),
                     CCCYN(ch, C_NRM), affected2_bits_desc[i], CCNRM(ch, C_NRM));
            strcat_s(buf, sizeof(buf), buf2);
        }
        page_string(ch->desc, buf);
        break;
    case 13:
        ...
        for (i = 0; i < NUM_AFF3_FLAGS; i++) {
            snprintf(buf2, sizeof(buf2), "  %s         %s%s%s\r\n", tmp_asciiflag(1U << i),
                     CCCYN(ch, C_NRM), affected3_bits_desc[i], CCNRM(ch, C_NRM));
            strcat_s(buf, sizeof(buf), buf2);
        }
        page_string(ch->desc, buf);
        break;
    ...
    case 33: /** vehicles **/
        ...
        for (i = 0; i < NUM_VEHICLE_TYPES; i++) {
            snprintf(buf2, sizeof(buf2), "%4d         %s%s%s\r\n",
                     (1U << i), CCCYN(ch, C_NRM), vehicle_types[i], CCNRM(ch,
                                                                         C_NRM));
            strcat_s(buf, sizeof(buf), buf2);
        }
        page_string(ch->desc, buf);
    ...
    case 34: /** engines **/
        ...
        for (i = 0; i < NUM_ENGINE_FLAGS; i++) {
            snprintf(buf2, sizeof(buf2), "%4d         %s%s%s\r\n",
                     (1U << i), CCCYN(ch, C_NRM), engine_flags[i], CCNRM(ch, C_NRM));
            strcat_s(buf, sizeof(buf), buf2);
        }
        page_string(ch->desc, buf);
        break;
