...
//
// save_zcmd(  )
// writes a single zcmd to file
//

int save_zcmd( struct char_data *ch, struct reset_com *zcmd, FILE *zone_file ) {
...
int save_zone( struct char_data *ch, struct zone_data *zone )
{
    ...
    sprintf( temp_fname,"world/zon/%d.zon.tmp", zone->number );
    sprintf( real_fname,"world/zon/%d.zon", zone->number );

    if ( !( zone_file = fopen( temp_fname,"w" ) ) )
	return 1;
	...
	sprintf( buf, "SYSERR: Error copying %s -> %s in save_zon: %s.", temp_fname, real_fname, strerror( errno ) );
	slog( buf );
	send_to_char( "There was an error copying the temporary .zon file during the save.\r\n"
		      "Zone Save Failed.\r\n", ch );
	return 1;
    }
