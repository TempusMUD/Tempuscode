...
do_olc_xset(struct creature *ch, char *argument)
{
    ...
    if (!*arg1) {
        snprintf(buf, sizeof(buf), "Valid xset commands:\r\n%s", CCYEL(ch, C_NRM));
...
void
format_search_data(struct str_builder *sb,
                   struct creature *ch,
                   ...
                   struct special_search_data *cur_search)
{
    ...
    sb_sprintf(sb, "%sCommand triggers:%s %s, %skeywords:%s %s\r\n",
    ...
    case SEARCH_COM_OBJECT:
        ...
        sb_sprintf(sb, "OBJECT Vnum : %d (%s%s%s), to room: %d, Max: %d.\r\n",
                    cur_search->arg[0],
                    CCGRN(ch, C_NRM),
    ...
    case SEARCH_COM_REMOVE:
        obj = real_object_proto(cur_search->arg[0]);
        sb_sprintf(sb, "REMOVE  Obj Vnum : %d (%s%s%s), Room # : %d, Val 2: %d.\r\n",
    ...
    case SEARCH_COM_DAMAGE:
        sb_sprintf(sb, "DAMAGE      %5d        %5d        %5d (%s)\r\n",
                    cur_search->arg[0],
                    cur_search->arg[1], cur_search->arg[2],
    ...
    case SEARCH_COM_SPAWN:
        sb_sprintf(sb, "SPAWN  Spawn_rm: %5d   Targ_rm:%5d   Hunt: %5d\r\n",
                    cur_search->arg[0], cur_search->arg[1], cur_search->arg[2]);
        break;
    case SEARCH_COM_LOADROOM:
        sb_sprintf(sb, "LOADROOM  NewLoad: %5d    MaxLevel:%5d    %5d\r\n",
                    cur_search->arg[0], cur_search->arg[1], cur_search->arg[2]);
        break;

    default:
        sb_sprintf(sb, "ERROR (%d)  %5d        %5d        %5d\r\n",
                    cur_search->command, cur_search->arg[0],
                    cur_search->arg[1], cur_search->arg[2]);
        break;
    }
    sb_sprintf(sb, "Flags: %s%s%s\r\n",
                CCBLU_BLD(ch, C_NRM),
