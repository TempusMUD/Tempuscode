...
struct shop_data *do_create_shop(struct char_data *ch, int vnum)
{
    ...
    new_shop->no_such_item1 = strdup("I don't have that...type LIST to see that I do have.");
    ...
    new_shop->missing_cash2 = strdup("You don't have enough money to buy this, sorry.");
...
void do_shop_sedit(struct char_data *ch, char *argument)
{
	...
	if (found == 0)
	    ...
	    if (!CAN_EDIT_ZONE(ch, zone)) {
		send_to_char("You do not have permission to edit this shop.\r\n",ch);
		return;
	    }
	    if ( ! OLC_EDIT_OK( ch, zone, ZONE_SHOPS_APPROVED ) ) {
		send_to_char("Shop OLC is not approved for this zone.\r\n", ch);
		...
		if (d->character && GET_OLC_SHOP(d->character) == tmp_shop) {
		    act("$N is already editing that shop.",FALSE,ch,0,d->character,
			TO_CHAR);
		    return;
		}
	    }
...
save_shops(struct char_data *ch)
{
	...
	if (!(file = fopen(fname,"r")))  {
	    ...
	    send_to_char("OLC Error: Failure to duplicate shop file in main dir."
			 "\r\n", ch);
	    fclose(realfile);
	    return 0;
	}
