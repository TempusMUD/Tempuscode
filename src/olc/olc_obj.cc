...
save_objs (struct char_data *ch)
{
	...
	while (desc != NULL)  {
		...
		sprintf(buf,"OLCERROR: Extra Desc with no kywrd or desc, obj #%d.\n",
			obj->shared->vnum);
		slog(buf);
		sprintf(buf, "I didn't save your bogus extra desc in obj %d.\r\n",
			obj->shared->vnum);
		send_to_char(buf, ch);
		desc = desc->next;
		continue;
	...
	if (!(file = fopen(fname,"r")))  {
	    ...
	    send_to_char("OLC Error: Failure to duplicate obj file in main dir."
			 "\r\n", ch);
	    fclose(realfile);
	    return 1;
	}
	     ...
	     char *argument, byte subcmd)
{
    ...
    for ( zone = zone_table; zone; zone = zone->next )
	if ( GET_OBJ_VNUM( obj_p ) >= zone->number*100 && GET_OBJ_VNUM( obj_p ) <= zone->top )
	    break;
    if ( !zone ) {
	sprintf( buf, "OLC: ERROR finding zone for object %d.", GET_OBJ_VNUM( obj_p ) );
	slog( buf );
	send_to_char( "Unable to match object with zone error..\r\n", ch );
	return;
    }
	...
	break;
    case 3:
	if (!(desc = locate_exdesc(fname(obj_p->name),
				   obj_p->ex_description))) {
	...
	if (!is_number(arg1)) {
		...
		if (!*arg2) {
		    ...
		    sprintf(buf, "Value %d (%s) set to %d.\r\n", i, 
			    item_value_types[(int)obj_p->obj_flags.type_flag][i], j);
		    send_to_char(buf, ch);
	...
	if (k >= MAX_OBJ_AFFECT) {
	    if (j)
		send_to_char("All apply slots are filled.  Set an existing apply mod. to zero to remove.\r\n", ch);
	    else
	...
	if (is_abbrev(arg1, "idnum"))
	    ...
	    sprintf(buf, "Unknown argument '%s'.  You must set sigil idnum or level.\r\n", arg2);
	    send_to_char(buf, ch);
	    return;
	}
	...
	if (!obj_p->obj_flags.material &&
	    (oset_command == 0 ||            // alias
	     oset_command == 1 ||            // name
