...
struct ticl_data * do_create_ticl(struct char_data *ch, int vnum)
{
    ...
    if (!zone) {
	send_to_char("ERROR: A zone must be defined for the TICL proc first.\r\n", ch);
	return(NULL);
    }
  
    if (!CAN_EDIT_ZONE(ch, zone)) {
	send_to_char("You can't touch this zone, buttmunch.\r\n", ch);
	sprintf(buf, "OLC: %s failed attempt to CREATE TICL %d.", GET_NAME(ch), vnum);
...
void do_ticl_tedit(struct char_data *ch, char *argument)
{
	...
	if (tmp_ticl == NULL)
		...
		if (j < zone->top)
		    break;
	    if (!zone) {
		send_to_char("That TICL proc does not belong to any zone!!\r\n", ch);
		slog("SYSERR: mobile not in any zone.");
		return;
	    }
	
	    if (!CAN_EDIT_ZONE(ch, zone)) {
		send_to_char("You do not have permission to edit those TICL procs.\r\n",ch);
		return;
	    }
		...
		if (d->character && GET_OLC_TICL(d->character) == tmp_ticl) {
		    act("$N is already editing that TICL proc.",FALSE,ch,0,d->character,
			TO_CHAR);
		    return;
		}
	    }
...
void do_ticl_tset(struct char_data *ch, char *argument)
{
...
int write_ticl_index(struct char_data *ch, struct zone_data *zone)
{
...
save_ticls(struct char_data *ch)
{
	...
	if (!(file = fopen(fname,"r")))  {
	    ...
	    send_to_char("OLC Error: Failure to duplicate TICL file in main dir."
			 "\r\n", ch);
	    fclose(realfile);
	    return 0;
	}
