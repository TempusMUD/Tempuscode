...
int check_room_cstrings( struct room_data *room )
{
    ...
    for ( struct special_search_data *search = room->search; search; search = search->next ) {
...
int save_room( struct char_data *ch, struct room_data *room, FILE *file )
{
    unsigned int                     i, j;
    unsigned int tmp;
    ...
    for ( i = 0; i < NUM_DIRS; i++ )  {
        if ( room->dir_option[i] )  {
            ...
            if ( room->dir_option[i]->general_description )  {
                ...
                for( j=0;j<tmp;j++ )
                    if ( room->dir_option[i]->general_description[j] != '\r' )
                        fputc( room->dir_option[i]->general_description[j], file );
            }
            fprintf( file, "~\n" );
            ...
            fprintf( file, "%d %d\n", room->dir_option[i]->key, 
                     room->dir_option[i]->to_room ? room->dir_option[i]->to_room->number :
                     ( -1 ) );
        }
    }
    ...
    while ( search ) {
        ...
        if ( search->to_remote ) {
            ...
            for ( i = 0; i < tmp; i++ )
                if ( search->to_remote[i] != '\r' && search->to_remote[i] != '~' )
...
save_wld ( struct char_data *ch )
{
    ...
    if ( rename( temp_fname, real_fname ) ) {
        sprintf( buf, "SYSERR: Error copying %s -> %s in save_wld: %s.", temp_fname, real_fname, strerror( errno ) );
        slog( buf );
        send_to_char( "There was an error copying the temporary .wld file during the save.\r\n"
                      "World Save Failed.\r\n", ch );
        return 1;
    }
...
do_olc_rset( struct char_data *ch, char *argument )
{
            ...
            if ( *arg3 ) {
                if ( !is_number( arg3 ) ) {
                    if ( ( i = search_block( arg3, flow_types, 0 ) ) < 0 ) {
                        send_to_char( "Invalid flow type... type olc h rflow.\r\n", ch );
                        return;
                    }
                } else
                    i = atoi( arg3 );
    ...
    default:
        send_to_char( "Sorry, you have attempted to access an unimplemented command.\r\nUnfortunately, you will now be killed.\r\n", ch );
        return;
    }
}
