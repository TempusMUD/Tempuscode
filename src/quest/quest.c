...
save_quest(struct quest *quest, FILE *out)
{
    ...
    fprintf(out, "%s<Quest VNUM=\"%d\" NAME=\"%s\" OWNER=\"%ld\" STARTED=\"%d\" "
            ...
            quest->awarded,
            quest->penalized,
            xmlEncodeTmp(qtype_abbrevs[quest->type]),
            quest->owner_level,
            quest->flags,
            quest->loadroom);
...
remove_quest_player(struct quest *quest, int id)
{
	...
	if (!(vict = get_char_in_world_by_idnum(player->idnum))) {
		...
		if (vict) {
			...
			errlog("Error loading player id %d from file for removal from quest %d.\r\n",
                   id, quest->vnum );
			return false;
		}
	} else {
...
char *
compose_qcomm_string(struct creature *ch, struct creature *vict, struct quest * quest, int mode, const char *str)
{
...
do_quest_ignore(struct creature *ch, char *argument)
{
	...
	send_to_char(ch, "Ok, you are %s ignoring '%s'.\r\n",
                 IS_SET(player->flags, QP_IGNORE) ? "now" : "no longer",
                 quest->name);
}
...
do_qcontrol_unmute(struct creature *ch, char *argument, int com)
{
    ...
    sprintf(buf, "unmuted %s in quest %d '%s'.", arg1, quest->vnum, quest->name);
	qlog(ch, buf, QLOG_COMP, 0, true);
...
do_qcontrol_award(struct creature *ch, char *argument, int com)
{
	...
	if ((ch) && (vict)) {
		...
		account_set_quest_points(vict->account, vict->account->quest_points + award);
		quest->awarded += award;
		save_player_to_xml(ch);
		save_player_to_xml(vict);
...
do_qcontrol_penalize(struct creature *ch, char *argument, int com)
{
	...
	if ((ch) && (vict)) {
		account_set_quest_points(vict->account, vict->account->quest_points - penalty);
