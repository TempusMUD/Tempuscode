...
real_path(char *str)
{
    ...
    for (path_head = first_path; path_head; path_head = (PHead *)path_head->next) 
...
add_path(char *spath, int save)
{
	...
	case 'e':
	case 'E':
	    if (!buf[1] || (buf[1] != '"') || !(tmpc = strchr((char *) spath, '"'))) {
		free(phead->path);
		free(phead);
#ifdef DMALLOC
		dmalloc_verify(0);
...
Load_paths(void)
{
	...
	case 5:
	    ...
	    break;
	default:
	    sprintf(buf,"SYSERR: Path %d in path position %d.",line,ret-5);
	    fail = 1;
	}
...
path_activity (void)
{
	...
	case PATH_ECHO:
	    ...
	    if (o->type == PMOBILE) {
		path_do_echo((struct char_data *) o->object, NULL, o->phead->path[o->pos].str);
	    }
	    else {
		path_do_echo(NULL, (struct obj_data *) o->object, o->phead->path[o->pos].str);
	    }
	    if (o->phead->length != 1)
		PATH_MOVE(o);
	    break;
