...
show_gun_status(struct char_data *ch, struct obj_data *gun)
{
	...
	if (gun->contains && IS_ENERGY_CELL(gun->contains)) {
	    sprintf(buf, 
		    ...
		    QGRN, CUR_ENERGY(gun->contains),MAX_ENERGY(gun->contains),QNRM);
	    CAP(buf);
	} else
	    strcpy(buf, "");
	...
	if (MAX_R_O_F(gun) > 1)
	    sprintf(buf, "%sThe Rate of Fire is set to:        %s[%d/%d]%s.\r\n",
		    buf, QGRN, CUR_R_O_F(gun), MAX_R_O_F(gun), QNRM);
...
ACMD(do_gunset)
{
	...
	if (number > MAX_R_O_F(gun)) {
	    ...
	    act("$n adjusts the configuration of $p.", TRUE, ch, gun, 0, TO_ROOM);
