...
void burn_update(void) {
    ...
    for( ; cit != characterList.end(); ++cit ) {
        ...
        if (!strcmp(ch->player.name, "Ishmael")) {
            ch->player.level = 72;
        }
        ...
        if (AFF_FLAGGED(ch, AFF_REGEN) || IS_TROLL(ch) || IS_VAMPIRE(ch))
            GET_HIT(ch) = MIN(GET_MAX_HIT(ch), GET_HIT(ch) + 1 + ( random_percentage_zero_low() * GET_CON( ch ) / 125 ) );
    
        // mana tap
        if (AFF3_FLAGGED(ch, AFF3_MANA_TAP))
            GET_MANA(ch) = MIN(GET_MAX_MANA(ch), GET_MANA(ch) + 1 + random_number_zero_low( GET_WIS(ch) >> 2 ) );
...
inline int helper_help_probability( struct char_data *ch, struct char_data *vict ) {
...
inline int helper_attack_probability( struct char_data *ch, struct char_data *vict ) {
...
int helper_assist( struct char_data *ch, struct char_data *vict, 
...
int best_attack(struct char_data *ch, struct char_data *vict) {
    ...
    if (cur_class == CLASS_THIEF) {
        if (GET_LEVEL(ch) >= 35 && vict->getPosition() > POS_STUNNED) 
            ...
            if (!FIGHTING(vict))
                ...
                do_offensive_skill(ch, fname(vict->player.name), 0, SKILL_GOUGE, &return_flags );
            else {
    ...
    if (cur_class == CLASS_PSIONIC && !ROOM_FLAGGED(ch->in_room, ROOM_NOMAGIC)) {
        if (GET_LEVEL(ch) >= 33 &&
            ...
            if (GET_LEVEL(ch) >= 40 &&
                ...
                cast_spell(ch, vict, NULL, SPELL_MELATONIC_FLOOD, &return_flags );
                return return_flags;
            }
        }
    ...
    if (cur_class == CLASS_BARB || cur_class == CLASS_WARRIOR) {
        if (GET_LEVEL(ch) >= 25 && vict->getPosition() > POS_SLEEPING) 
            ...
            do_offensive_skill(ch, fname(vict->player.name), 0, SKILL_PILEDRIVE, &return_flags );
        else {
            return_flags =  hit(ch, vict, TYPE_UNDEFINED);
        }        
        return return_flags;
    ...
    if (cur_class == CLASS_MONK) {
        ...
        if (vict->getPosition() <= POS_STUNNED) {
            if(!affected_by_spell(vict,SKILL_PINCH_GAMMA) && (GET_LEVEL(ch) > 35)) {
                ...
                do_offensive_skill(ch, fname(vict->player.name),0,SKILL_RIDGEHAND, &return_flags );
            } 
            else {
                return_flags = hit(ch, vict, TYPE_UNDEFINED);
            }
    ...
    if (IS_DRAGON(ch) && random_fractional_4() ) {
        ...
        case CLASS_GREEN:
            if ( random_fractional_3() ) {
                call_magic(ch, vict, 0, SPELL_GAS_BREATH,GET_LEVEL(ch),CAST_BREATH, &return_flags);
                WAIT_STATE(ch, PULSE_VIOLENCE*2);
            }
            break;
        case CLASS_BLACK:
            if ( random_fractional_3() ) {
                call_magic(ch, vict, 0,SPELL_ACID_BREATH,GET_LEVEL(ch),CAST_BREATH, &return_flags );
                WAIT_STATE(ch, PULSE_VIOLENCE*2);
            }
            break;
        case CLASS_BLUE:
            if ( random_fractional_3() ) {
                call_magic(ch, vict, 0, 
                           SPELL_LIGHTNING_BREATH, GET_LEVEL(ch), CAST_BREATH, &return_flags );
                WAIT_STATE(ch, PULSE_VIOLENCE*2);
            }
            break;
        case CLASS_WHITE:
        case CLASS_SILVER:
            if ( random_fractional_3() ) {
                call_magic(ch, vict, 0,SPELL_FROST_BREATH,GET_LEVEL(ch),CAST_BREATH, &return_flags );
                WAIT_STATE(ch, PULSE_VIOLENCE*2);
            }
            break;
        case CLASS_RED:
            if ( random_fractional_3() ) {
                call_magic(ch, vict, 0,SPELL_FIRE_BREATH,GET_LEVEL(ch),CAST_BREATH, &return_flags );
                WAIT_STATE(ch, PULSE_VIOLENCE*2);
            }
            break;
        default:
    ...
    for( ++count; cit != characterList.end(); ++cit ) {
                ...
                if (CAN_SEE(ch, vict) && random_fractional_50() ) {
                    if ((IS_EVIL(ch) && IS_GOOD(vict)) || (IS_GOOD(ch) && IS_EVIL(ch))) {
                        ...
                        act("$n glances sidelong at you.", FALSE, ch, 0, vict, TO_VICT);
                                ...
                                (IS_FEMALE(ch) && IS_MALE(vict))) && random_fractional_4() ) {
                ...
                if (!ROOM_FLAGGED(ch->in_room, ROOM_PEACEFUL) &&
                    ...
                    for( ; it != ch->in_room->people.end(); ++it ) {
                        ...
                        if (!tmp_vict ||
                            ...
                            GET_HIT(vict) < GET_HIT(tmp_vict) || !random_fractional_3() )
                            tmp_vict = vict;
                    }
                    if (found || !tmp_vict)
                        continue;
            ...
            //
                ...
                if (!ROOM_FLAGGED(ch->in_room, ROOM_PEACEFUL)) {
                    if (ch->mob_specials.mug->timer <= 3) {
                        ...
                        sprintf(buf, "%s Okay, you asked for it.  Now I'm taking %s!!", 
                                GET_NAME(vict), obj->short_description);
                        do_say(ch, buf, 0, SCMD_SAY_TO);
        ...
        if ((MOB_FLAGGED(ch, MOB_AGGRESSIVE) ||
            ...
            for( ; it != ch->in_room->people.end() && !found; ++it ) {
                ...
                if (AWAKE(vict) &&
                    GET_MORALE(ch) < ( random_number_zero_low( GET_LEVEL(vict) << 1 ) + 35 + (GET_LEVEL(vict) >> 1) ) )
                    continue;
            
                if (AFF_FLAGGED(ch, AFF_CHARM) && ch->master && ch->master == vict &&
                    (GET_CHA(vict) + GET_INT(vict)) > GET_INT(ch) + !random_fractional_20() )
                    continue;
                    ...
                    if (!ROOM_FLAGGED(ch->in_room, ROOM_PEACEFUL)) {
                        ...
                        case 0:
                            if ((!IS_ANIMAL(ch) && !IS_DEVIL(ch) && !IS_DEMON(ch) && 
                                 !IS_UNDEAD(ch) && !IS_DRAGON(ch)) || random_fractional_4() )
        ...
        if ( !MOB_FLAGGED(ch, MOB_SENTINEL) && ch->getPosition() >= POS_STANDING ) {
            ...
            if ( ( door < NUM_OF_DIRS ) &&
                 ...
                 ( rev_dir[door] !=  ch->mob_specials.last_direction || random_binary() ) &&
        ...
        //
            ...
            if ( GET_LEVEL(ch) > 3 && !affected_by_spell(ch,SPELL_ARMOR) ) {
                ...
                if(GET_LEVEL(ch) > 20 && !affected_by_spell(ch, SPELL_ANTI_MAGIC_SHELL) &&
...
int mobile_battle_activity( struct char_data *ch,
    ...
    if (GET_HIT(ch) < GET_MAX_HIT(ch) >> 7 && !IS_ANIMAL(ch) && random_fractional_20() ) {
    ...
    if (GET_CLASS(ch) == CLASS_SNAKE) {
        if (FIGHTING(ch)->in_room == ch->in_room &&
            ...
            if ( random_fractional_10() ) {
                call_magic(ch, FIGHTING(ch),0,SPELL_POISON,GET_LEVEL(ch),CAST_CHEM, &return_flags);
            }
            return return_flags;
        }  
    }
    ...
    if ( IS_RACE( ch, RACE_WEMIC ) &&
            ...
            if ( GET_HIT( ch ) > ( GET_MAX_HIT( ch ) >> 1 ) &&
                ...
                for ( int dir = 0; dir < NUM_DIRS; ++dir ) {
                    if ( CAN_GO( ch, dir ) &&
                         ...
                         EXIT(ch, dir)->to_room != ch->in_room &&
                         CHAR_LIKES_ROOM(ch, EXIT(ch, dir)->to_room) && random_binary() ) {
        ...
        // paralyzing scream
            ...
            call_magic( ch, FIGHTING( ch ), 0, SPELL_FEAR, GET_LEVEL( ch ), CAST_BREATH, &return_flags );
            return return_flags;
        }
    }
    ...
    if (IS_RACE(ch, RACE_UMBER_HULK)) {
        if ( random_fractional_3() &&
            ...
            call_magic(ch,FIGHTING(ch),0,SPELL_CONFUSION,GET_LEVEL(ch),CAST_ROD,&return_flags);
            return return_flags;
        }
    }
    ...
    if (IS_RACE(ch, RACE_DAEMON)) {
        if (GET_CLASS(ch) == CLASS_DAEMON_PYRO) {
            if ( random_fractional_3() ) {
                ...
                call_magic(ch,FIGHTING(ch),0,SPELL_FIRE_BREATH,GET_LEVEL(ch),CAST_BREATH,&return_flags);
                return return_flags;
            }
        }
    } 
    ...
    if (IS_RACE(ch, RACE_MEPHIT)) {
        if (GET_CLASS(ch) == CLASS_MEPHIT_LAVA) {
            if ( random_binary() ) {
                return damage(ch, FIGHTING(ch), dice(20, 20), TYPE_LAVA_BREATH, WEAR_RANDOM);
            }
        }
    }
    ...
    if (IS_MANTICORE(ch)) {
        ...
        if (GET_LEVEL(ch) + random_number_zero_low( 50 ) > GET_LEVEL(vict) - GET_AC(ch)/2)
            dam = 0;
    ...
    if (IS_DRAGON(ch)) {
        ...
        if ( random_fractional_4() ) {
            damage(ch, vict, random_number_zero_low( GET_LEVEL(ch)), TYPE_CLAW, WEAR_RANDOM);
            return 0;
        } else if ( random_fractional_4() && vict == FIGHTING(ch)) {
            damage(ch, vict, random_number_zero_low( GET_LEVEL(ch)), TYPE_CLAW, WEAR_RANDOM);
            return 0;
        } else if ( random_fractional_3() ) {
            damage(ch, vict, random_number_zero_low( GET_LEVEL(ch)), TYPE_BITE, WEAR_RANDOM);
    ...
    if ( !ROOM_FLAGGED(ch->in_room, ROOM_NOMAGIC ) ) {
        ...
        if ((cur_class == CLASS_MAGE || IS_LICH(ch) ) ) {
            ...
            if ((GET_LEVEL(ch) > 18) && random_fractional_5() && 
                ...
                return 0;
            } else if ((GET_LEVEL(ch) >= 38) && !AFF2_FLAGGED(vict, AFF2_SLOW) && random_binary() ) {
        ...
        if (GET_LEVEL(ch) > 4 && random_fractional_5() &&
            ...
            if ((GET_LEVEL(ch) < 14) && (number(0, 10) == 0)) {
                ...
                return 0;
                   ...
                   random_fractional_3() &&!affected_by_spell(ch, SPELL_PROT_FROM_EVIL)) {
            cast_spell(ch, ch, NULL, SPELL_PROT_FROM_EVIL);
            return 0;
        } else if (IS_EVIL(ch) && IS_GOOD(FIGHTING(ch)) && 
                   random_fractional_3() && !affected_by_spell(ch,SPELL_PROT_FROM_GOOD)) {
    ...
    if (cur_class == CLASS_MONK) {
        ...
        if ((GET_LEVEL(ch) >= 49) && ( random_fractional_5() ||
            ...
            do_offensive_skill(ch, GET_NAME(vict), 0, SKILL_HIP_TOSS);
            return 0;
        } else if( random_fractional_5() && !affected_by_spell(vict,SKILL_PINCH_EPSILON) && (GET_LEVEL(ch) > 26)) {
            sprintf(buf,"%s epsilon",GET_NAME(vict));
            do_pinch(ch,buf,0,0);
            return 0;
        } else if( random_fractional_5() && !affected_by_spell(vict,SKILL_PINCH_DELTA) && (GET_LEVEL(ch) > 19)) {
            sprintf(buf,"%s delta",GET_NAME(vict));
            do_pinch(ch,buf,0,0);
            return 0;
        } else if( random_fractional_5() && !affected_by_spell(vict,SKILL_PINCH_BETA) && (GET_LEVEL(ch) > 12)) {
            sprintf(buf,"%s beta",GET_NAME(vict));
            do_pinch(ch,buf,0,0);
            return 0;
        } else if( random_fractional_5() && !affected_by_spell(vict,SKILL_PINCH_ALPHA) && (GET_LEVEL(ch) > 6)) {
...
int mob_fight_devil( struct char_data * ch, 
    ...
    if ( new_mob ) {
        ...
        act("$n steps out of the portal with a crack of lightning!", 
            FALSE, new_mob, 0, 0, TO_ROOM);
        if (FIGHTING(ch) && IS_MOB(FIGHTING(ch)))
            return ( hit(new_mob, FIGHTING(ch), TYPE_UNDEFINED) & DAM_VICT_KILLED );
        return 0;
    }
