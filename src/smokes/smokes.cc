...
struct obj_data *roll_joint(struct obj_data *tobac, struct obj_data *paper)
{
    struct obj_data *obj;
...
ACMD(do_roll)
{
    ...
    if (!*arg1) {
	send_to_char("You roll your eyes in disgust.\r\n", ch);
	...
	send_to_char("That's not smoking material!\r\n", ch);
    else if (!*arg2)
	act("What would you like to roll $p in?", FALSE, ch, tobac, 0, TO_CHAR);
...
ACMD(do_smoke)
{
    ...
    if (!*arg1) {
	send_to_char("Smoke blows out of your ears.\r\n", ch);
	...
	if (!(vict = get_char_room_vis(ch, arg1))) 
	    send_to_char("Smoke who or what?\r\n", ch);
	else if (vict == ch)
	    send_to_char("Hmmm... okay.\r\n", ch);
	else {
	    act("You blow smoke in $N's face.", FALSE, ch, 0, vict, TO_CHAR);
	    act("$n blows smoke in your face.", FALSE, ch, 0, vict, TO_VICT);
	    act("$n blows smoke in $N's face.", FALSE, ch, 0, vict, TO_NOTVICT);
	}
    } else if (GET_OBJ_TYPE(joint) == ITEM_TOBACCO)
	send_to_char("You need to roll that up first.\r\n", ch);
	...
	if (GET_OBJ_VAL(joint, 3)) {
		...
		act("You burn your finger as you smoke the last of $p.", FALSE, ch, joint, 0, TO_CHAR);
		act("$n burns $s finger on $p.", FALSE, ch, joint, 0, TO_ROOM);
		extract_obj(joint);
	    }
	    perform_smoke(ch, type);
	} else
	    send_to_char("It doesn't seem to be lit.\r\n", ch);
	...
	if (GET_OBJ_VAL(joint, 3)) {
		...
		act("You inhale a burning ash as you finish $p.", FALSE, ch, joint, 0, TO_CHAR);
		act("$n coughs harshly after inhaling the comet from $p.", FALSE, ch, joint, 0, TO_ROOM);
...
ACMD(do_convert)
{
	...
	sprintf(buf, "You can't seem to have %s '%s'.\r\n", AN(argument),argument);
	send_to_char(buf, ch);
	...
	send_to_char("You cannot convert that!\r\n", ch);
    else if (GET_OBJ_TYPE(obj) == ITEM_PIPE)
	send_to_char("It's already a pipe.\r\n", ch);
    else {
...
ACMD(do_light)
{
	...
	send_to_char("You need to roll it up first.\r\n", ch);
	...
	if (IS_BOMB(obj)) {
	    ...
	    send_to_char("That's already lit.\r\n", ch);
...
ACMD(do_extinguish)
{
	...
	if (!IS_AFFECTED_2(ch, AFF2_ABLAZE))
	    send_to_char("You're not even on fire!\r\n", ch);
	else if (((number(1, 80) > (GET_LEVEL(ch) + GET_WIS(ch) - GET_AC(ch)/10))
	    ...
	    send_to_char("You fail to extinguish yourself!\r\n", ch);
	    act("$n tries to put out the flames which cover $s body, but fails!", 
		FALSE, ch, 0, 0, TO_ROOM);
	} else {
	    send_to_char("You succeed in putting out the flames.  WHEW!\r\n", ch);
	...
	if (!OBJ_TYPE(ovict, ITEM_PIPE) &&
	    !OBJ_TYPE(ovict, ITEM_CIGARETTE) &&
	    !OBJ_TYPE(ovict, ITEM_BOMB))
	    ...
	    send_to_char("That's not ablaze!\r\n", ch);
	else {
	...
	else if (number(40, 80) > (GET_LEVEL(ch) + GET_WIS(ch) - GET_AC(vict)/10)) {
	    act("You fail to extinguish $M!",FALSE, ch, 0, vict, TO_CHAR);
	    act("$n tries to put out the flames on $N's body, but fails!", FALSE, ch, 0, vict, TO_NOTVICT);
	    act("$n tries to put out the flames on your body, but fails!", FALSE, ch, 0, vict, TO_VICT);
	} else {
	    send_to_char("You succeed in putting out the flames.  WHEW!\r\n", ch);
	    act("$n hastily puts out the flames on your body.", FALSE, ch, 0, vict, TO_VICT);
	    act("$n hastily extinguishes the flames on $N's body.", FALSE, ch, 0, vict, TO_NOTVICT);
...
ACMD(do_ignite)
{
	...
	send_to_char("That's probably a really bad idea.\r\n", ch);
	...
	if (ch != vict)
	    act("$N is suddenly consumed in burning flames!!!", FALSE, ch, 0, vict, TO_CHAR);
	act("$N is suddenly consumed in burning flames!!!", FALSE, ch, 0, vict, TO_NOTVICT);
	act("You are suddenly consumed in burning flames!!!", FALSE, ch, 0, vict, TO_VICT);
