		...
		struct affected_type *af)
{
    ...
    prob -= ((IS_WEARING_W(ch) + IS_CARRYING_W(ch)) * 15) /
	CAN_CARRY_W(ch);
	...
	if (GET_EQ(ch, WEAR_WIELD))
	    prob += (LEARNED(ch) - weapon_prof(ch, GET_EQ(ch, WEAR_WIELD))) >> 2;
	...
	*loc = WEAR_EYES;
	...
	if ((GET_WEIGHT(vict) + ((IS_CARRYING_W(vict)+IS_WEARING_W(vict)) >> 1)) > 
	    CAN_CARRY_W(ch)*1.5) {
	    act("$N is too heavy for you to lift!", FALSE, ch, 0, vict, TO_CHAR);
	    act("$n tries to pick you up and piledrive you!",
	...
	*move = 20;
	...
	if ((!weap || !SLASHING(weap)) &&
	    (!(weap = GET_EQ(ch, WEAR_HANDS)) || 
	     ...
	     !SLASHING(weap))) {
	    send_to_char("You need to wield a good slashing weapon to do this.\r\n",
			 ch);
	    return -1;
	}
	...
	*move = 10;
    ...
    default:
	sprintf(buf, "SYSERR: Illegal skillnum <%d> passed to calc_skill_prob().",
		skillnum);
	slog(buf);
	...
	send_to_char("You lack the spiritual energies needed!\r\n", ch);
	return -1;
    }
...
ACMD(do_order)
{
	...
	if (vict) {
	    send_to_char(CCBLD(vict, C_SPR), vict);
	    ...
	    send_to_char(CCBLD(ch, C_SPR), ch);
	    send_to_char(CCRED(ch, C_NRM), ch);
	    sprintf(buf, "You order %s to '%s'.\r\n", PERS(vict, ch), message);
	    send_to_char(buf, ch);
	    ...
	    send_to_char(CCBLD(ch, C_SPR), ch);
	    send_to_char(CCRED(ch, C_NRM), ch);
	    sprintf(buf, "You order your followers to '%s'.\r\n", message);
	    send_to_char(buf, ch);
	    send_to_char(CCNRM(ch, C_SPR), ch);
		...
		if (org_room == k->follower->in_room)
			...
			GET_LEVEL(ch) >= LVL_CREATOR) {
			found = TRUE;
			if (!CHECK_WAIT(k->follower) && !GET_MOB_WAIT(k->follower)) {
			    if (IS_NPC(k->follower) && GET_MOB_VNUM(k->follower) == 5318)
				do_say(vict, "As you command, master.", 0, 0);
			    command_interpreter(k->follower, message);
			}
		    } else 
			act("$n has an indifferent look.", TRUE,k->follower,0,0,TO_CHAR);
	    }
	    order_next_k = NULL;
...
ACMD(do_retreat)
{
    ...
    if (GET_MOVE(ch) < 10) {
	send_to_char("You are too exhausted to make an effective retreat.\r\n", ch);
	return;
    }
    if (!*argument) {
	...
	send_to_char("No such direction, fool.\r\n", ch);
	return;
    }
	...
	if (FIGHTING(ch)) {
		...
		if (percent < number(100, 170)) {
		    ...
		    sprintf(buf, "$n bashes the %s open with a powerful blow!", 
			    ...
			    fname(EXIT(ch, door)->keyword) : "door");
		    act(buf, FALSE, ch, 0, 0, TO_ROOM);
		    sprintf(buf, "The %s gives way under your powerful bash!\r\n", 
			    EXIT(ch, door)->keyword ? 
			    fname(EXIT(ch, door)->keyword) : "door");
			...
			EXIT(ch, door)->to_room->dir_option[rev_dir[door]]->to_room
			== ch->in_room) {
			sprintf(buf, "The %s is bashed open from the other side!!\r\n", 
				EXIT(ch, door)->to_room->dir_option[rev_dir[door]]->keyword ?
				fname(EXIT(ch, door)->to_room->dir_option[rev_dir[door]]->keyword) :
				"door");
			REMOVE_BIT(EXIT(ch, door)->to_room->dir_option[rev_dir[door]]->exit_info, EX_CLOSED);
			REMOVE_BIT(EXIT(ch, door)->to_room->dir_option[rev_dir[door]]->exit_info, EX_LOCKED);
...
ACMD(do_stun)
{
	...
	send_to_char("You are not particularly stunning.\r\n", ch);
	return;
    }
    if (!(vict = get_char_room_vis(ch, arg))) {
	if (FIGHTING(ch)) {
	    ...
	    send_to_char("Who would you like to stun?\r\n", ch);
	    WAIT_STATE(ch, 4);
	    return;
	}
    }
    if (vict == ch) {
	send_to_char("Aren't we stunningly funny today...\r\n", ch);
	return;
    }
    if (FIGHTING(ch)) {
    ...
    if (FIGHTING(vict)) {
	send_to_char("You aren't able to get the right grip!\r\n", ch);
	return;
    }
    if (!peaceful_room_ok(ch, vict, true))
	return;
...
ACMD(do_tag)
{
    ...
    if (!(vict = get_char_room_vis(ch, arg))) {
	send_to_char("Who do you want to tag in?\r\n", ch);
	return;
    }
    if (!FIGHTING(ch)) {
	send_to_char("There is no need.  You aren't fighting!\r\n", ch);
	return;
    }
    if (vict == ch) {
	send_to_char("Okay! You tag yourself in!...dummy.\r\n", ch);
	return;
    }
    if (FIGHTING(ch) == vict) {
	...
	if (percent > prob) {
	    send_to_char("You fail the tag!\r\n", ch);
	    act("$n tries to tag you into the fight, but fails.", FALSE, vict, 0, ch, TO_CHAR);
	    return;
	}
	prob = CHECK_SKILL(vict, SKILL_TAG);
	if (percent > prob) {
	    act("You try to tag $N, but $E misses the tag!", FALSE, ch, 0, vict, TO_CHAR);
	    act("$n tries to tag you, but you miss it!", FALSE, vict, 0, ch, TO_CHAR);
	    return;
	} else {
	    act("You tag $N!  $E jumps into the fray!", FALSE, ch, 0, vict, TO_CHAR);
...
ACMD(do_tornado_kick)
{
	...
	if (FIGHTING(ch)) {
	    ...
	    act("You spin into the air, kicking $p!", FALSE, ch, ovict, 0, TO_CHAR);
	    act("$n spins into the air, kicking $p!", FALSE, ch, ovict, 0, TO_ROOM);
	    return;
	} else {
	    send_to_char("Upon who would you like to inflict your tornado kick?\r\n", ch);
	    WAIT_STATE(ch, 4);
	    return;
	}
    }
...
ACMD(do_turn)
{
	...
	GET_REMORT_CLASS(ch) != CLASS_CLERIC && GET_REMORT_CLASS(ch) != CLASS_KNIGHT &&
        GET_LEVEL(ch) < LVL_AMBASSADOR) {
	send_to_char("Heathens are not able to turn the undead!\r\n", ch);
	return;
    }
    one_argument(argument, arg);
    ...
    if (vict == ch) {
	send_to_char("You try to flee from yourself, to no avail.\r\n", ch);
	return;
    }
    if (!IS_NPC(vict)) {
	send_to_char("You cannot turn other players!\r\n", ch);
	return;
    }
    if (!IS_UNDEAD(vict)) {
	send_to_char("You can only turn away the undead.\r\n", ch);
	return;
    }
...
ACMD(do_shoot)
{
    ...
    if (!*arg) {
	send_to_char("Shoot what at who?\r\n", ch);
	return;
    }
	...
	if (!*arg) {
	    send_to_char("Discharge which implant at who?\r\n", ch);
	    return;
	}
	...
	if (!(vict = FIGHTING(ch))) {
	    ...
	    if (IS_GUN(gun) && !IS_ARROW(gun) && GUN_TYPE(gun) >= 0 && GUN_TYPE(gun) < NUM_GUN_TYPES) {

		if (!(bullet = gun->contains) || (!MAX_LOAD(gun) && !(bullet = bullet->contains)))
		    act("$p is not loaded.", FALSE, ch, gun, 0, TO_CHAR);
		else {
		    act("$n fires $p into the air.", FALSE, ch, gun, 0, TO_ROOM);
		    act("You fire $p into the air.", FALSE, ch, gun, 0, TO_CHAR);
	...
	if (!gun->contains || !IS_ENERGY_CELL(gun->contains)) {
	    act("$p is not loaded with an energy cell.",FALSE,ch,gun,0,TO_CHAR);
	    return;
	}
	...
	if (IS_ARROW(gun)) {
	    sprintf(buf, "$n fires %s from $p into $P!", bullet->short_description);
...
ACMD(do_ceasefire)
{
	...
	send_to_char("You can't ceasefire while your enemy is actively attacking you.\r\n", ch);
    else {
...
ACMD(do_disarm)
{
	...
	send_to_char("They aren't wielding anything, fool.\r\n", ch);
	return;
    }
	...
	act("$n knocks $p out of $N's hand!", FALSE, ch, weap, vict, TO_NOTVICT);
	...
	if (GET_STR(ch) + number (0, 20) > GET_STR(vict) + GET_DEX(vict)) {
	    ...
	    if (IS_NPC(vict) && !GET_MOB_WAIT(ch) && AWAKE(ch) &&
	...
	send_to_char("You fail the disarm!\r\n", ch);
...
ACMD(do_intimidate)
{
	...
	if (FIGHTING(ch)) {
		...
		act("You attempt to terrorize $p!", FALSE, ch, ovict, 0, TO_CHAR);
		act("$n attempts to terrorize $p!", FALSE, ch, ovict, 0, TO_ROOM);
	    } else {
	    ...
	    sprintf(buf, "%s who?\r\n", subcmd == SKILL_TERRORIZE ? "Terrorize" :
		    "Intimidate");
	    send_to_char(buf, ch);
	    WAIT_STATE(ch, 4);
	    return;
    ...
    if (!CAN_SEE(vict, ch)) {
	act("$N doesn't seem to be able to see you.",FALSE,ch,0,vict,TO_CHAR);
	return;
    }
    if (vict == ch) {
	if (subcmd == SKILL_TERRORIZE) {
	    send_to_char("You cannot succeed at this.\r\n", ch);
	    return;
	}

	send_to_char("You attempt to intimidate yourself.\r\n"
	...
	if (affected_by_spell(vict, SKILL_TERRORIZE)) {
	    act("$n PANICS and attempt to escape!", FALSE, vict, 0, 0, TO_ROOM);
	    send_to_char("You PANIC and attempt to escape!\r\n", vict);
	    if (!MOB_FLAGGED(vict, MOB_SENTINEL))
		do_flee(vict, "", 0, 0);

	    act("$n cowers in fear!", TRUE, vict, 0, 0, TO_ROOM);
	    send_to_char("You cower in paralyzing fear!\r\n", vict);
	    ...
	    act("$N glarse at $n with contempt.", TRUE, ch, 0, vict, TO_NOTVICT);
	    act("$N glares at you with contempt!", TRUE, ch, 0, vict, TO_CHAR);
	    send_to_char("You glare back with contempt!\r\n", vict);
	    if (IS_NPC(vict))
		hit(vict, ch, TYPE_UNDEFINED);
	}
	WAIT_STATE(ch, PULSE_VIOLENCE);
...
ACMD(do_drain)
{
	...
	if (FIGHTING(ch)) {
	    vict = FIGHTING(ch);
		    ...
		    get_obj_in_list_vis(ch, arg, ch->in_room->contents))) {
	    send_to_char("You can't drain objects!\r\n", ch);
	    return;
	} else {
	    send_to_char("Drain whos life force?\r\n", ch);
	    WAIT_STATE(ch, 4);
	    return;
	}
    }
...
ACMD(do_beguile)
{
	...
	send_to_char("Beguile who??\r\n", ch);
	WAIT_STATE(ch, 4);
	return;
    }
    if (vict == ch) {
	send_to_char("You find yourself amazingly beguiling.\r\n", ch);
	return;
    }
	...
	send_to_char("There appears to be no effect.\r\n", ch);
