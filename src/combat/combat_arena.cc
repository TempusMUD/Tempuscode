...
do_ccontrol_options(CHAR *ch)
{
    ...
    if(GET_LEVEL(ch) >= LVL_GOD) {
        ...
        while (1) {
            ...
            sprintf(buf, "%s  %s%-15s %s%s%s\r\n",
                    buf, KCYN, cc_wizoptions[j].keyword, KYEL, cc_wizoptions[j].usage, KNRM);
            }
...
do_ccontrol_show(CHAR *ch, char *argument)
{
    ...
    *(timestr_s + strlen(timestr_s) - 1) = '\0';
    ...
    sprintf(buf, 
            ...
            KCYN, KYEL, combat->sacrificed ? "Yes" : "No", KNRM);
...
do_ccontrol_join(CHAR *ch, char* argument)
{
    ...
    if(the_combat->type == CTYPE_ONE_ONE && !idnum_in_combat(GET_IDNUM(ch), the_combat)) {
        if(the_combat->num_players >= 2) {
            sprintf(buf, "Battle '%s' is a one-on-one battle and is currently full.\r\n", the_combat->name);
            send_to_char(buf, ch);
            return;
        }
    }

    if(COMBAT_FLAGGED(the_combat, COMBAT_INVITE) && GET_IDNUM(ch) != the_combat->creator) {
    ...
    if(COMBAT_FLAGGED(the_combat, COMBAT_OPEN) || GET_IDNUM(ch) == the_combat->creator){  
...
do_ccontrol_open(CHAR *ch) {
    ...
    if(GET_IDNUM(ch) != combat->creator) {
        send_to_char("Only the battle creator can open a battle to the public.\r\n", ch);
        return;
    }
...
do_ccontrol_start(CHAR *ch) {
    ...
    if(combat->arena->zone == 0) {
        send_to_char("You have to choose an arena before you start genius.\r\n", ch);
        return;
    }
    ...
    case CTYPE_FREE_FOR_ALL:
        if(combat->num_players < 2) {
            send_to_char("You need at least two combatants for a free for all.\r\n", ch);
            return;
        }
        break;
    default:
    ...
    if(COMBAT_FLAGGED(combat, COMBAT_STARTED) ) {
        send_to_char("The battle is already started...better check yourself before you wreck yourself.\r\n", ch);
        return;
    }
    else {
        REMOVE_BIT(combat->flags, COMBAT_OPEN);
...
do_ccontrol_describe(CHAR *ch) {
    ...
    if(COMBAT_FLAGGED(combat, COMBAT_STARTED)) {
        send_to_char("The combat has already started...no need to describe it now.\r\n", ch);
        return;
    }
    ...
    if(combat) {
        if ( combat->description ) {
            act( "$n begins to edit a combat description.", TRUE, ch, 0, 0, TO_ROOM );
        } else {
            act( "$n begins to write a combat description.", TRUE, ch, 0, 0, TO_ROOM );
        }
...
do_ccontrol_lock(CHAR* ch) 
{
    ...
    sprintf(buf, "Combat System %s by %s", combat_on ? "unlocked" : "LOCKED", GET_NAME(ch));
    slog(buf);
    ...
    send_to_char(buf, ch);
            ...
            if(!combat_on) {
                for(the_combat = battles;;)
                {
                    ...
                    send_to_combat("The combat system has temporarily been locked.\r\n", the_combat);
...
do_ccontrol_fee(struct char_data *ch, char* arg)
{
    ...
    if(combat->num_players > 1 ) {
        send_to_char("The combat must have only one combatant to set the fee.\r\n", ch);
        return;
    }
...
int check_battles(combat_data *combat) {
    ...
    case 1:     // One on One
        if(combat->num_players == 1) { // There can be only ONE!
            ...
            sprintf(buf, "Battle '%s' won by %s", combat->name, get_name_by_id(combat->winner));
...
random_arena_room(struct combat_data *combat,CHAR* ch)
{
    ...
    {     
        ...
        if(room_num == combat->arena->booty_room || room_num == BATTLE_START_ROOM) {
        ...
        if(trans_room){
            ...
            if( ROOM_FLAGGED(trans_room, ROOM_NOTEL) || ROOM_FLAGGED(trans_room, ROOM_GODROOM)) {
                random_arena_room(combat, ch);
            }
...
say_to_combat(char* argument, CHAR* ch, combat_data *combat) 
{
    ...
    sprintf(buf, "%sYou grunt,%s '%s'%s\r\n'",KCYN_BLD, KNRM_BLD, argument, KNRM);
    send_to_char(buf, ch);

    sprintf(buf2, "%s%s grunts,%s '%s'%s\r\n",KCYN_BLD, GET_NAME(ch), KNRM_BLD, argument, KNRM);
    
    for (i = 0; i < combat->num_players; i++) {
    
        if ((ch2 = get_char_in_world_by_idnum(combat->players[i].idnum)) && !(ch2 == ch)) {
        ...
        if(the_arena) {
            for (zn = zone_table; zn; zn = zn->next) {
                if (zn->number == the_arena->zone) {
                    sprintf(buf, "%s%4d) %s%30s      %20s%s\r\n",KCYN, i,KYEL, zn->name,(the_arena->used ? "(unavailable)" : "(available)"), KNRM);
                    send_to_char(buf, ch);
                }
            }
        }
...
combat_loop(CHAR* ch, CHAR* killer)
{
    ...
    if(combat) {
        ...
        if(!killer) {
            ...
            remove_idnum_from_combat(GET_IDNUM(ch), combat);
            sprintf(buf, "%s killed by %s in battle '%s'", GET_NAME(ch), GET_NAME(killer), battles->name);
            send_to_combat(buf, combat);
