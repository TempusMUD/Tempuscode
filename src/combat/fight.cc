       ...
       int attacktype, int location)
{
    ...
    if ((af = affected_by_spell(victim, SPELL_BARKSKIN)) ||
    ...
    if (ch && ch != victim && ch != FIGHTING(victim)) {
	...
	if (dam) {
		...
		affect_from_char(victim, SPELL_CONFUSION);
	}
	...
	if (!IS_DEFENSE_ATTACK(attacktype)) {
