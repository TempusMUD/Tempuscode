...
make_corpse(struct creature *ch, struct creature *killer, int attacktype)
{
	...
	case SKILL_PISTOLWHIP:
		corpse->line_desc = strdup(tmp_sprintf("The bruised up %s of %s %s lying here.",
                                               typebuf, GET_NAME(ch), isare));
		strcpy(adj, "bruised");
		break;

	case TYPE_STING:
		corpse->line_desc = strdup(tmp_sprintf("The bloody, swollen %s of %s %s lying here.",
                                               typebuf, GET_NAME(ch), isare));
		strcpy(adj, "stung");
		break;

	case TYPE_WHIP:
		corpse->line_desc = strdup(tmp_sprintf("The scarred %s of %s %s lying here.",
                                               typebuf, GET_NAME(ch), isare));
		strcpy(adj, "scarred");
		break;
	...
	case SPELL_BLADE_BARRIER:
		corpse->line_desc = strdup(tmp_sprintf("The chopped up %s of %s %s lying here.",
                                               typebuf, GET_NAME(ch), isare));
		strcpy(adj, "chopped up");
		break;

	case SONG_WOUNDING_WHISPERS:
		corpse->line_desc = strdup(tmp_sprintf("The perforated %s of %s %s lying here.",
                                               typebuf, GET_NAME(ch), isare));
		strcpy(adj, "perforated");
		break;

	case SKILL_HAMSTRING:
		corpse->line_desc = strdup(tmp_sprintf("The legless %s of %s %s lying here.",
                                               typebuf, GET_NAME(ch), isare));
		strcpy(adj, "legless");
		...
		leg->line_desc = strdup(tmp_sprintf("The severed %sleg of %s %s lying here.",
			AFF2_FLAGGED(ch, AFF2_PETRIFIED) ? "stone " : "", GET_NAME(ch),
			isare));
		leg->name = strdup(tmp_sprintf("the severed %sleg of %s",
			AFF2_FLAGGED(ch, AFF2_PETRIFIED) ? "stone " : "", GET_NAME(ch)));
	...
	case SKILL_BITE:
	case TYPE_BITE:
		corpse->line_desc = strdup(tmp_sprintf("The chewed up looking %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "chewed up");
		break;

	case SKILL_SNIPE:
		corpse->line_desc = strdup(tmp_sprintf("The sniped %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "sniped");
		break;
	...
	case TYPE_POUND:
	case TYPE_PUNCH:
		corpse->line_desc = strdup(tmp_sprintf("The battered %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "battered");
		break;
	...
	case SPELL_PSYCHIC_CRUSH:
		corpse->line_desc = strdup(tmp_sprintf("The crushed %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "crushed");
		break;

	case TYPE_CLAW:
		corpse->line_desc = strdup(tmp_sprintf("The shredded %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "shredded");
		break;

	case TYPE_MAUL:
		corpse->line_desc = strdup(tmp_sprintf("The mauled %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "mauled");
		break;

	case TYPE_THRASH:
		corpse->line_desc = strdup(tmp_sprintf("The %s of %s %s lying here, badly thrashed.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "thrashed");
		break;

	case SKILL_BACKSTAB:
		corpse->line_desc = strdup(tmp_sprintf("The backstabbed %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
    ...
    case TYPE_EGUN_PARTICLE:
		corpse->line_desc = strdup(tmp_sprintf("The bloody %s of %s %s lying here, full of holes.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "stabbed");
		break;

	case TYPE_GORE_HORNS:
		corpse->line_desc = strdup(tmp_sprintf("The gored %s of %s %s lying here in a pool of blood.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "gored");
		break;

	case TYPE_TRAMPLING:
		corpse->line_desc = strdup(tmp_sprintf("The trampled %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "trampled");
		break;

	case TYPE_TAIL_LASH:
		corpse->line_desc = strdup(tmp_sprintf("The lashed %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "lashed");
		break;
    ...
    case TYPE_EGUN_LASER:
		corpse->line_desc = strdup(tmp_sprintf("The blasted %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "blasted");
		break;

	case SKILL_PROJ_WEAPONS:
		corpse->line_desc = strdup(tmp_sprintf("The shot up %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "shot up");
		break;

	case SKILL_ARCHERY:
		corpse->line_desc = strdup(tmp_sprintf("The pierced %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "pierced");
		break;
    ...
    case TYPE_EGUN_PLASMA:
		corpse->line_desc = strdup(tmp_sprintf("The charred %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "charred");
		break;
    ...
    case TYPE_EGUN_ION:
		corpse->line_desc = strdup(tmp_sprintf("The smoking %s of %s %s lying here,",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "smoking");
		break;

	case TYPE_BOILING_PITCH:
		corpse->line_desc = strdup(tmp_sprintf("The scorched %s of %s %s here.", typebuf, GET_NAME(ch),
			isare));
		strcpy(adj, "scorched");
		break;

	case SPELL_STEAM_BREATH:
		corpse->line_desc = strdup(tmp_sprintf("The scalded %s of %s %s here.", typebuf, GET_NAME(ch),
			isare));
		strcpy(adj, "scalded");
		break;
	...
	case SPELL_FROST_BREATH:
		corpse->line_desc = strdup(tmp_sprintf("The frozen %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "frozen");
		break;
	...
	case SPELL_EARTH_ELEMENTAL:
		corpse->line_desc = strdup(tmp_sprintf("The smashed %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "smashed");
		break;
	...
	case TYPE_RIP:
		corpse->line_desc = strdup(tmp_sprintf("The ripped apart %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "ripped apart");
		break;

	case SPELL_WATER_ELEMENTAL:
		corpse->line_desc = strdup(tmp_sprintf("The drenched %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "drenched");
		break;
    ...
    case TYPE_EGUN_GAMMA:
		corpse->line_desc = strdup(tmp_sprintf("The radioactive %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "radioactive");
		break;

	case SPELL_ACIDITY:
		corpse->line_desc = strdup(tmp_sprintf("The sizzling %s of %s %s lying here, dripping acid.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "sizzling");
		break;

	case SPELL_GAS_BREATH:
		corpse->line_desc = strdup(tmp_sprintf("The %s of %s lie%s here, stinking of chlorine gas.",
			typebuf, GET_NAME(ch), ISARE(typebuf)));
		strcpy(adj, "chlorinated");
		break;

	case SKILL_TURN:
		corpse->line_desc = strdup(tmp_sprintf("The burned up %s of %s %s lying here, finally still.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "burned");
		break;

	case TYPE_FALLING:
		corpse->line_desc = strdup(tmp_sprintf("The splattered %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
	...
	case SKILL_PILEDRIVE:
		corpse->line_desc = strdup(tmp_sprintf("The shattered, twisted %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		...
		if (GET_MOB_VNUM(ch) == 1511) {
			...
			if (AFF2_FLAGGED(ch, AFF2_PETRIFIED)) {
				...
				spine->line_desc = strdup("A stone spinal column is lying here.");
				spine->name = strdup("a stone spinal column");
				GET_OBJ_VAL(spine, 1) = 4;
			} else {
				spine->aliases = strdup("spine spinal column bloody");
				spine->line_desc = strdup("A bloody spinal column is lying here.");
	...
	case TYPE_TAINT_BURN:
		if (IS_RACE(ch, RACE_BEHOLDER) || NON_CORPOREAL_MOB(ch)) {
			...
			// attack that rips the victim's head off
			corpse->line_desc = strdup(tmp_sprintf("The headless smoking %s of %s %s lying here.",
				typebuf, GET_NAME(ch), isare));
	...
	case SKILL_CLOTHESLINE:
		corpse->line_desc = strdup(tmp_sprintf("The headless %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		...
		head->line_desc = strdup(tmp_sprintf("The severed %shead of %s %s lying here.",
			AFF2_FLAGGED(ch, AFF2_PETRIFIED) ? "stone " : "", GET_NAME(ch),
			isare));
		head->name = strdup(tmp_sprintf("the severed %shead of %s",
			AFF2_FLAGGED(ch, AFF2_PETRIFIED) ? "stone " : "", GET_NAME(ch)));
	...
	case SKILL_LUNGE_PUNCH:
		corpse->line_desc = strdup(tmp_sprintf("The maimed %s of %s %s lying here.", typebuf,
			GET_NAME(ch), isare));
		strcpy(adj, "maimed");
		...
		if (AFF2_FLAGGED(ch, AFF2_PETRIFIED)) {
			...
			heart->name = strdup(tmp_sprintf("the stone heart of %s", GET_NAME(ch)));
		} else {
			GET_OBJ_TYPE(heart) = ITEM_FOOD;
			heart->aliases = strdup("heart bloody");
			heart->name = strdup(tmp_sprintf("the bloody heart of %s", GET_NAME(ch)));
		}

		heart->line_desc = strdup(tmp_sprintf("The %sheart of %s %s lying here.",
			AFF2_FLAGGED(ch, AFF2_PETRIFIED) ? "stone " : "", GET_NAME(ch),
			isare));
	...
	case SKILL_IMPALE:
		corpse->line_desc = strdup(tmp_sprintf("The run through %s of %s %s lying here.",
			typebuf, GET_NAME(ch), isare));
		strcpy(adj, "impaled");
		break;
	...
	case TYPE_DROWNING:
		...
		corpse->line_desc = strdup(tmp_sprintf("The waterlogged %s of %s %s lying here.", typebuf,
				GET_NAME(ch), ISARE(typebuf)));
		else
		corpse->line_desc = strdup(tmp_sprintf("The %s of %s %s lying here.", typebuf,
				GET_NAME(ch), ISARE(typebuf)));
		strcpy(adj, "drowned");
		break;
	...
	if (attacktype != TYPE_SWALLOW)
        corpse->name = strdup(tmp_sprintf("the %s%s%s of %s", adj, *adj ? " " : "", typebuf,
                                          GET_NAME(ch)));
	else
