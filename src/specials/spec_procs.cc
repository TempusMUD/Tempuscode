        ...
        if (ahole && IS_MALE(ch)) {
            ...
            for( ; it != ch->in_room->people.end(); ++it ) {
                if ((*it) != ch && IS_FEMALE((*it)) && CAN_SEE(ch, (*it))) {
                    sprintf(buf, "%s Excuse me, ma'am.", fname(GET_NAME((*it))));
...
SPECIAL(cityguard)
{
    ...
    for( ; it != ch->in_room->people.end(); ++it ) {
        ++nit;
        if (!IS_NPC((*it)) && CAN_SEE(ch, (*it)) && !PRF_FLAGGED((*it),PRF_NOHASSLE)) {
            if (IS_SET(PLR_FLAGS((*it)), PLR_KILLER) && (nit != ch->in_room->people.end() || 
                                                       !number(0, 2))) {
                max_evil = 1;
                tch = (*it);
                break;
            }
            if (IS_SET(PLR_FLAGS((*it)), PLR_THIEF) && (nit != ch->in_room->people.end() || 
                                                      !number(0, 2))) {
                max_evil = 0;
                tch = (*it);
                break;
