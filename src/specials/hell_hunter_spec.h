...
class Hunter {
    public:
        ...
        Hunter(xmlNodePtr n, vector<Devil> &devils) {
            ...
            xmlChar *c = xmlGetProp(n, (const xmlChar *)"Name");
            if( c != NULL ) {
                vector<Devil>::iterator it = find(devils.begin(),devils.end(),c);
                if( it != devils.end() )
                    m_vnum = (*it).vnum;
                free(c);
            }
...
class HuntGroup : public std::vector<Hunter> {
    public:
        HuntGroup(xmlNodePtr n, vector<Devil> &devils ) : std::vector<Hunter>() {
            level = xmlGetIntProp(n,"Level");
            n = n->xmlChildrenNode;
            while (n != NULL) {
        ...
        int level;
        bool operator<(const HuntGroup &h) const { return level < h.level;}
        bool operator>(const HuntGroup &h) const { return level > h.level;}
        bool operator==(const HuntGroup &h) const { return level == h.level;}
        bool operator!=(const HuntGroup &h) const { return level != h.level;}
};
