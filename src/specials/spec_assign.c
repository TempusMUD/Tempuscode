...
const struct spec_func_data spec_list[] = {
    ...
    {"wand_of_wonder", wand_of_wonder, SPEC_OBJ},
    ...
    {"artisan", artisan, SPEC_MOB | SPEC_RES},
...
assign_objects(void)
{
    ...
    while (!feof(file) && !ferror(file)) {
        ...
        if (!(obj = real_object_proto(vnum))) {
            if (!mini_mud) {
                slog("Error in obj spec file: object <%d> not exist.", vnum);
            }
        } else if ((index = find_spec_index_arg(ptr_name)) < 0) {
            slog("Error in obj spec file: ptr <%s> not exist.", ptr_name);
...
assign_rooms(void)
{
    ...
    while (!feof(file) && !ferror(file)) {
        ...
        if (!(rm = real_room(vnum))) {
            if (!mini_mud) {
                slog("Error in room spec file: room <%d> not exist.", vnum);
            }
        } else if ((index = find_spec_index_arg(ptr_name)) < 0) {
            slog("Error in room spec file: ptr <%s> not exist.", ptr_name);
