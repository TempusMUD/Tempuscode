...
/*30*/
void clear_quest(void)
{
  quest_rec.min = -1;
  quest_rec.max = -1;
  quest_rec.number = -1;
  if (quest_rec.key_wrd != NULL)   free (quest_rec.key_wrd);
  if (quest_rec.quest_desc != NULL) free (quest_rec.quest_desc);
  if (quest_rec.first_clue != NULL) free (quest_rec.first_clue);
}
/*40*/
SPECIAL(darom)
{
  ...
  if (strncasecmp(buf, "darom", 5)) return 0;
  ...
  if (!strncasecmp(buf2, "assignment", 10))
      ...
      for (i=0; i<num_quest;i++)
	{
	  ...
	  if (( quest_rec.max >= GET_LEVEL(ch) )&&(quest_rec.min <= GET_LEVEL(ch)))
	    {
