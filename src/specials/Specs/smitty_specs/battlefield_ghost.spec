...
SPECIAL(battlefield_ghost)
{
  ...
  if (OUTSIDE(ch) && ch->in_room != r_ghost_hole && 
    ...
    if (number(0, 2)) {
      if (!number(0, 3))
        act("$n looks to the sun with sorrow.",TRUE, ch, 0, 0, TO_ROOM);
      else if (!number(0, 2))
        act("$n seems to recoil from the sun.", TRUE, ch, 0, 0, TO_ROOM);
      else 
    ...
    act("$n has disappeared into the netherworld.", FALSE, ch, 0, 0, TO_ROOM);
    return 1;
    ...
    act("$n seems to rise out of the pile of bones.", FALSE,ch,0, 0, TO_ROOM);
    char_from_room(ch);
