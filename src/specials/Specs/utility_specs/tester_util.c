...
SPECIAL(tester_util)
{
    ...
    if (!*arg1) {
        act("$p: ERROR: Activate what?", false, ch, obj, NULL, TO_CHAR);
    ...
    case 0:                    /* advance */
        if (!*arg2)
            ...
            act("$p: Advance: I DON'T THINK SO!", false, ch, obj, NULL, TO_CHAR);
        else {
            if (i < GET_LEVEL(ch)) {
                do_start(ch, true);
                GET_LEVEL(ch) = i;
            ...
            for (i = 1; i < MAX_SKILLS; i++) {
                if (spell_info[i].min_level[(int)GET_CLASS(ch)] <=
                    GET_LEVEL(ch)
                    || (IS_REMORT(ch)
                        && spell_info[i].min_level[(int)CHECK_REMORT_CLASS(ch)]
                        <= GET_LEVEL(ch))) {
