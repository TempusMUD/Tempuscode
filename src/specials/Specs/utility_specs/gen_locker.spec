...
SPECIAL(gen_locker)
{
  ...
  if (CMD_IS("store")) {
    ...
    act("$n takes all your things and locks them in a locker.", FALSE, atten, 0, ch, TO_VICT);
  ...
  if (CMD_IS("receive")) {
    ...
    for (locker=locker_room->contents;locker;locker=locker->next_content) {
      if (GET_OBJ_VNUM(locker) != locker_vnum || 
	  GET_OBJ_VAL(locker, 0) != GET_IDNUM(ch)
	  || (!locker->contains)) 
        continue;
    ...
    if (GET_OBJ_VAL(locker, 1)) {
      ...
      hours = CONVERT_TIME_TO_CONT(time_info) - GET_OBJ_VAL(locker, 2) + 1;
