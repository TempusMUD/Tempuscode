...
SPECIAL(jail_locker)
{
	...
	if (CMD_IS("receive")) {
		...
		if (GET_GOLD(ch) < cost && GET_LEVEL(ch) < LVL_IMMORT) {
			sprintf(buf2,
				"You don't have the %d gold coins bail money which I require.",
				cost);
		...
		for (locker = ch->in_room->next->contents; locker;
			...
			if (GET_OBJ_VNUM(locker) != v_locker ||
				GET_OBJ_VAL(locker, 0) != GET_IDNUM(ch) || (!locker->contains))
				continue;
			if (GET_OBJ_VNUM(locker) != v_locker ||
			...
			act("$n opens a locker and gives you all your things.", FALSE,
				atten, 0, ch, TO_VICT);
			act("$n opens a locker and gives $N all $S things.", FALSE, atten,
				0, ch, TO_NOTVICT);
			sprintf(buf2,
