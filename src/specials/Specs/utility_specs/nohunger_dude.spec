...
SPECIAL(nohunger_dude)
{
  ...
  if (!*argument) {
    ...
    if (!is_abbrev(argument, "nohunger")) {
      if (!is_abbrev(argument, "nothirst")) {
        if (!is_abbrev(argument, "nodrunk")) {
          perform_tell(dude, ch, "You can only gain 'nohunger', 'nothirst', or 'nodrunk'.");
          return 1;
        } else 
          mode = DRUNK;
      } else 
    ...
    if (GET_GOLD(ch) < gold) {
      sprintf(buf, "You don't have the %d gold coins I require for that.", gold);
      perform_tell(dude, ch, buf);
    } else if (GET_LIFE_POINTS(ch) < life_cost) {
      sprintf(buf, "You haven't gained the %d life points you need to do that.", life_cost);
      perform_tell(dude, ch, buf);
    } else {
