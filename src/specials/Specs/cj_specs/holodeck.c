    ...
    if (CMD_IS("computer")) {
        ...
        if (!strncasecmp(arg2, "program", 4)) {
            if (!strcasecmp(arg1, "save")) {
                if (!zone) {
                    send_to_char
                        ("HC tells you, 'A program is not currently running.'\r\n",
                        ch);
                    return TRUE;
                }
                if (holo_savezone(ch, zone))
                    ...
                    sprintf(buf,
                        "HC tells you, 'Error occurred while saving program %d.'\r\n",
                        zone->zonenum);
                send_to_char(buf, ch);
                return TRUE;
                ...
                if (zone) {
                    send_to_char
                        ("HC tells you, 'This Holodeck is already running a program.'\r\n",
                        ch);
                    return TRUE;
                }
                if (!(zone = holo_newzone(ch))) {
                    send_to_char
                        ("HC tells you, 'A new program could not be registered.'\r\n",
                        ch);
                    return TRUE;
                }
                ...
                if (holo_savezone(ch, zone)) {
                    sprintf(buf,
                        "HC tells you, 'Program %d saved and terminated.'\r\n",
                        zone->zonenum);
                ...
                sprintf(buf,
                    "HC tells you, 'Discarding changes and closing program %d.'\r\n",
                    zone->zonenum);
                send_to_char(buf, ch);
                holo_freezone(zone->zonenum);
                return TRUE;
                ...
                send_to_char
                    ("HC tells you, 'Unable to list your known programs.'\r\n",
                    ch);
                return TRUE;
            } else {
            ...
            if (!strcasecmp(arg1, "room")) {
                ...
                send_to_char
                    ("HC tells you, 'Mobile editing mode currently unavailable.'\r\n",
                    ch);
                return TRUE;
            } else if (!strcasecmp(arg1, "object")) {
                send_to_char
                    ("HC tells you, 'Object editing mode currently unavailable.'\r\n",
                    ch);
                return TRUE;
            } else if (!strcasecmp(arg1, "zone")) {
                send_to_char
                    ("HC tells you, 'Zone editing mode currently unavailable.'\r\n",
                    ch);
                return TRUE;
            } else {
            ...
            if (holo_delzone(ch, num))
                ...
                sprintf(buf,
                    "HC tells you, 'Delete request for program %d failed'.\r\n",
                    num);
            send_to_char(buf, ch);
            return TRUE;
        } else {
...
HoloZone *
holo_findzone(struct room_data * room)
{
...
HoloZone *
holo_newzone(struct char_data * ch)
{
...
HoloZone *
holo_loadzone(struct char_data * ch, int num)
{
    HoloZone *zone;
    FILE *file;
    char buf[256], *err;
...
int
holo_savezone(struct char_data *ch, HoloZone * zone)
{
    FILE *file;
    char buf[256];
    char buf2[256];
...
int
holo_delzone(struct char_data *ch, int num)
{
    FILE *zonefile;
    char buf[256];
    char buf2[256];
...
holo_freezone(int num)
{
    ...
    if (FirstHoloZone->zonenum == num) {
        ...
        ztmp = FirstHoloZone;
        while ((ztmp->nextzone != NULL)
            && ((HoloZone *) ztmp->nextzone)->zonenum != num)
            ztmp = ztmp->nextzone;
...
char *
holo_loadwld(FILE * file, HoloZone * zone)
{
    HoloRoom *this;
    int nr = -1, last = 0;
    char line[256], *ch;
    ...
    while (true) {
        ...
        if (*line != '#') {
            sprintf(buf,
                "Format error in %s file near %s #%d\nOffending line: '%s'\r\n",
                mode, mode, nr, line);
            return buf;
        }
        last = nr;
...
int
holo_addroom(HoloRoom * this, HoloZone * zone)
{
    HoloRoom *tmp;
...
/* load a room */
char *
holo_parse_room(FILE * file, int virtual_nr, HoloRoom * room)
{
    int t[10], dir;
...
int
holo_fread_string(FILE * file, char *string, int maxlength)
{
    char tmp[512];
    char *point;
    int done = 0, length = 0, tmplen = 0;
...
int
holo_savewld(FILE * file, HoloZone * zone)
{
    ...
    while (this != NULL) {
        ...
        for (i = 0; i < 8; i++) {
            if (this->exits[i].exists) {
                ...
                fprintf(file, "%d %d %d\n", this->exits[i].door,
                    this->exits[i].key, this->exits[i].to_room);
            }
        }
