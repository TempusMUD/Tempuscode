...
SPECIAL(cyborg_overhaul)
{
  ...
  struct char_data *borg = (struct char_data *) me;
  int cost;
  ...
  if (!IS_CYBORG(ch)) {
    perform_tell(borg, ch, "I am not programmed to deal with non-cyborgs.");
    return 1;
  }
  ...
  if (!str_cmp(argument, "overhaul")) {
    ...
    if (CMD_IS("value")) {
      ...
      act("$n interfaces momentarily with $n.",TRUE,ch,0,borg,TO_ROOM);
      return 1;
    }
  ...
  if (!str_cmp(argument, "repairs")) {
    ...
    act("$N begins to perform repairs on you.", FALSE, ch, 0, borg, TO_CHAR);
    act("$N begins to perform repairs on $n.", FALSE, ch, 0, borg, TO_ROOM);
