...
SPECIAL(abandoned_cavern)
{
    ...
    if (count > 30) {
	count = 0;
	act("The cavern begins to shake, and rocks start falling from the ceiling!",
	    FALSE, ch, 0, 0, TO_ROOM);
	act("The cavern begins to shake, and rocks start falling from the ceiling!",
	    FALSE, ch, 0, 0, TO_CHAR | TO_SLEEP);

	for (vict = cavern->people; vict; vict = next_vict) {
	    next_vict = vict->next_in_room;

	    if (number(5, 30) > GET_DEX(vict) && GET_LEVEL(vict) < LVL_IMMORT) {
		act("A shower of rubble crushes $n!", FALSE, vict, 0, 0, TO_ROOM);
	...
	send_to_room("You see the glimmer of gold from deep within the cavern.\r\n", cavern);
    else if (!number(0, 15))
	send_to_room("You see some treasure shining deep in the cavern.\r\n", cavern);
    else if (!number(0, 14))
	send_to_room("You hear a lady calling for help deeper in the cavern.\r\n", cavern);
    else if (!number(0, 14))
	send_to_room("You notice sunlight coming from somewhere deeper in the cavern.\r\n", cavern);
