...
SPECIAL(horn_of_geryon)
{
  ...
  struct char_data *minotaur = NULL;
  
  if ((!CMD_IS("wind") && !CMD_IS("blow")) || GET_OBJ_TYPE(horn) == ITEM_PIPE)
    return 0;

  act("$n sounds a clear, ultra low note on $p.", FALSE, ch, horn, 0, TO_ROOM);
  act("You sound a clear, ultra low note on $p.", FALSE, ch, horn, 0, TO_CHAR);
