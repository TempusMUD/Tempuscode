SPECIAL(pit_keeper)
{
    ...
    if (GET_STR(ch) + number(0, 10) > GET_STR(vict)) {
        ...
        act("$n lifts $N and hurls $M into the pit below!", false, ch, NULL, vict,
            TO_NOTVICT);
        ...
        act("$n is hurled in from above!", false, vict, NULL, NULL, TO_ROOM);
        look_at_room(vict, vict->in_room, 0);
        WAIT_STATE(vict, 1 RL_SEC);
        return 1;
    } else {
        act("$n attempts to grapple with you and fails!", false, ch, NULL, vict,
            TO_VICT);
