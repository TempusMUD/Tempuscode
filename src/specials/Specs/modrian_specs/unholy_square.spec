...
void
perform_defile(struct room_data *room, int *state, char **olddesc, char **oldtitle)
{
...
void
perform_resanct(struct room_data *room, int *state, char *olddesc, char *oldtitle)
{
...
SPECIAL(unholy_square)
{
  ...
  if (CMD_IS("pour") && state == STATE_UNHOLY && ch->in_room->number == 3013) {
    ...
    if (!*arg1 || !(obj = GET_EQ(ch, WEAR_HOLD)) || GET_OBJ_VNUM(obj) != VIAL_VNUM ||
	!isname(arg1, obj->name) || !*arg2)
      return 0;
    ...
    act("$n pours the contents of $p into $P...", FALSE, ch, obj, fount, TO_ROOM);
    act("You pour the contents of $p into $P...", FALSE, ch, obj, fount, TO_CHAR);
    ...
    act("\r\nWhen the dust clears, $P is left standing in its place.", FALSE, ch, obj, fount, TO_ROOM);
    act("\r\nWhen the dust clears, $P is left standing in its place.", FALSE, ch, obj, fount, TO_CHAR);
  ...
  if (!strcmp(arg1, "begin")) {
    ...
    send_to_zone("A large black cloud rolls across the sky from the west, enveloping\r\n"
		 "the city in darkness.\r\n", ch->in_room->zone, 1);

    send_to_room("A whirling funnel bursts from the bottom of the black cloud with a piercing\r\n"
