        ...
        if (xmlMatches(node->name, "itemdata")) {
            ...
            for (xmlNodePtr cnode = node->xmlChildrenNode;
                ...
                if (xmlMatches(cnode->name, "object")) {
                    ...
                    if (!obj) {
                        errlog("Auctioneer failed to load item from %s",
                            fname);
                        extract_obj(obj);
                        continue;
                    }
...
auctioneer_tick(struct creature *self)
{
    ...
    for (GList * ai = items; ai; ai = ai->next, item_no++) {
        ...
        if (item->new_bid) {
            ...
            ((struct imp_data *)imp->mob_specials.func_data)->mode =
                IMP_DELIVER_ITEM;
...
SPECIAL(do_auctions)
{
    ...
    if (CMD_IS("auction") && ch != self) {
        ...
        struct obj_data *obj =
            get_obj_in_list_all(ch, item_name, ch->carrying);
        if (!obj) {
    ...
    if (CMD_IS("withdraw") && ch != self) {
        ...
        if (((item->current_bid != 0) || (item->buyer_id != 0))
            && !IS_IMMORT(ch)) {
