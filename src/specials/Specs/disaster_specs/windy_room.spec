...
boot_windy_rooms()
{
	...
	newdata = ( windy_room_data * ) malloc(sizeof(windy_room_data) * tot_good);
	for (i = 0, j = 0; i < num_windy_rooms; i++) {
	    if (windy_list[i].vnum >= 0) {
		newdata[j] = windy_list[i];
		j++;
		if (j >= tot_good) {
		    sprintf(buf, "SYSERR: major error in sanity checking booted windy list.");
		    free(newdata);
		    free(windy_list);
		    windy_list = NULL;
		    num_windy_rooms = 0;
...
int
immort_windy_command(CHAR *ch, windy_room_data *windy, char *argument)
{
