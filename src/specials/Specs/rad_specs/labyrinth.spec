...
SPECIAL(labyrinth_clock)
{
  ...
  if (!CMD_IS("enter") && !CMD_IS("wind")) return (0);
...
SPECIAL(cuckoo)
{
  ...
  if (room_status >= 7) {
    ...
    if (bird->in_room->number == 66104) {
      ...
      char_from_room(bird);
      char_to_room(bird, r_clock_room);
      act("$n suddenly pops out of the clock and says 'Cuckoo! Cuckoo!'.",
          TRUE, bird, 0, 0, TO_ROOM);
      return 1;
    }
  }
  return 0;
...
SPECIAL(pendulum_room)
{
  ...
  if (!CMD_IS("south") && !CMD_IS("north")) return 0;
...
if( spec_mode == SPECIAL_DEATH ) return 0;
  ...
  half_chop(argument,buf,buf2);
  if (strcasecmp(buf, GET_NAME(master))) return 0;
...
SPECIAL(astrolabe)
{
  ...
  if (ch != wearer || !IS_OBJ_TYPE(astrolabe, ITEM_OTHER)) return (0);
  if (!CMD_IS("adjust") && !CMD_IS("use")) return (0);

  skip_spaces(&argument);
  if (!isname(argument, astrolabe->name)) return 0;
  ...
  if (CMD_IS("adjust")) {
    ...
    act("$n lifts $p and peers through it.", TRUE, ch, astrolabe, 0, TO_ROOM);
    ...
    if (NUM_USES(astrolabe) <= 0) {
      act("$p slowly fades from existance.", FALSE, ch, astrolabe, 0, TO_CHAR);
