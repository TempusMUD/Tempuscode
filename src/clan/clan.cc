...
ACMD(do_demote)
{
    ...
    } else if (!PLR_FLAGGED(ch, PLR_CLAN_LEADER) && !Security::isMember(ch,"Clan" ) ) {
...
ACMD(do_cedit)
{
    ...
    if( !Security::isMember(ch,"Clan" ) ) {
        send_to_char("Sorry, you can't use the cedit command.\r\n", ch);
    }

    if ( cedit_keys[cedit_command].level == 1 && !Security::isMember(ch,"WizardFull" ) ) {
